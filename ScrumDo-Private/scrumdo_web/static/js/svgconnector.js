...
; (function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    connectElements: function (pathData) {
      ...
      if (orientation != "vertical" && orientation != "horizontal" && orientation != "curve" && orientation != "curve1" && orientation != "depcurve") {
      ...
      } else if (orientation != "depcurve") { // Horizontal
      ...
      if(orientation == "depcurve"){
        var startX = startCoord.left + $startElem.outerWidth() - svgLeft;
        var startY = startCoord.top + 10 - svgTop;
        var endX = endCoord.left - svgLeft;
        var endY = endCoord.top + ($endElem.outerHeight() - 10) - svgTop;
      }
    ...
    drawPath: function ($path, offset, orientation, startX, startY, endX, endY) {
      ...
      if (orientation == "vertical") {
        ...
        $path.attr("d", "M" + startX + " " + startY +
          ...
          " V" + endY);
      } else if(orientation == "curve") {
        // Curve
        var mid = Math.floor((startX + endX)/2);
        var distance = Math.abs((endX - startX)/3) > 50 ? 50 : Math.abs((endX - startX)/3);
        $path.attr("d", "M" + startX + " " + startY +
          " C " + (mid) + " " + (startY+distance) + " " + mid + " " + (endY+distance) + " " + endX + " " + endY);
      } else if(orientation == "curve1") {
        // BigPicture Curve
        var mid = Math.floor((startX + endX)/2);
        $path.attr("d", "M" + startX + " " + startY +
          " C " + (mid) + " " + (endY) + " " + mid + " " + (startY) + " " + endX + " " + endY);
      }
      else if(orientation == "depcurve") {
        // Story Dependency Box Curve
        var mid = Math.floor((startX + endX)/2);
        $path.attr("d", "M" + startX + " " + startY +
