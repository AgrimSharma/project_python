...
module scrumdo{
    ...
    export class BigPictureStatsController{
        ...
        constructor(private $scope: thisScope,
            ...
            this.workItemName = this.$scope.project.work_item_name;
        }
        ...
        loadStats(refresh = false){
            if(this.$scope.isroot){
                ...
                if(this.$scope.incrementid == null) return;
                this.loaded = false;
        ...
        loadSystemRisks(){
            if(!this.$scope.isroot){
                ...
                this.bigPictureManager.loadRootSystemRisks(this.$scope.project.slug, 
                                                            this.$scope.iterationid).then((risks) => {
                    _.forEach(risks, (risk:any) => {
                        this.systemRisks[risk.type].push(risk);
                    });
                });
        ...
        loadRisks(){
            if(!this.$scope.isroot){
                this.bigPictureManager.loadIncrementRisks(this.$scope.project.slug, 
                                                                this.$scope.incrementid).then((risks) => {
                    this.risks = risks;
                });
            }else{
                this.bigPictureManager.loadRootRisks(this.$scope.portfolioid, 
                                                    this.$scope.iterationid).then((risks) => {
                    this.risks = risks;
                });
            }
