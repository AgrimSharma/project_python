...
module scrumdo {
    export class TimeManager { 
        ...
        getTimeEntries(projectSlug: string, userId, startDate, endDate, iteration: Iteration = null): ng.IPromise<any> {
            startDate = moment(startDate).format("YYYY-MM-DD");
            endDate = moment(endDate).format("YYYY-MM-DD");
            let iterationID = iteration != null ? iteration.id : null;
            if (typeof projectSlug !== "undefined" && projectSlug !== null) {
                return this.ProjectTimeEntry.query({ projectSlug: projectSlug, 
                                                     user: userId, 
                                                     start: startDate, 
                                                     end: endDate, 
                                                     iterationID: iterationID }).$promise;
            } else {
                return this.OrgTimeEntry.query({ organizationSlug: 
        ...
        updateEntry(entry){
            var e = new this.OrgTimeEntry({ organizationSlug: this.organizationSlug});
            angular.extend(e, entry);
            return e.$save();
