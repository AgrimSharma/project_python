...
module scrumdo {
    ...
    export class BoardController {
        ...
        constructor(
            ...
            scope.$on("globalBacklogStoriesUpdate", this.onGlobalBacklogStoriesUpdate);
        ...
        setupShortcuts() {
            if (   (this.boardProject != null)
                ...
                && this.userService.canWrite(this.boardProject.project.slug)) {
        ...
        onGlobalBacklogStoriesUpdate = () => {
            this.globalBacklogStories = this.scope.$root['globalBacklogStories'] != null ?
        ...
        getSelectedStories() {
            ...
            if (this.scope.boardProject.uiState.loadGlobalBacklog) {
                if(this.globalBacklogStories != null){
                    return boardStories.concat(_.where(this.globalBacklogStories, { selected: true }));
                }
            }
            return boardStories;
        }
        ...
        getDefaultCellId() {
            if (this.boardProject.boardCells != null && this.boardProject.boardCells.length > 0) {
        ...
        _setupSortable = () => {
            ...
            for (i = 0, len = ref1.length; i < len; i++) {
                ...
                this.sortables.push(new Sortable(el, {
                    group: 'stories',
                    filter: ".task-view, .no-drag, .embedded-task-window, .task-view, .glyphicon-pencil",
                    draggable: ".cards",
        ...
        onSortStory = (event) => {
            ...
            if (this.getSelectedCount() === 0) {
                story = this._onSortSingleStory(storyId, item, parent, cellId, iterationId, placeholder, null, projectId, releaseId);
                stories = [story];
                ...
                for (var i = 0, len = stories.length; i < len; i++) {
                    story = stories[i];
                    this._onSortSingleStory(story.id, item, parent, cellId, iterationId, placeholder, previousStory, projectId, releaseId);
                    previousStory = story;
                }
                if(projectId != null && projectId !== story.project_slug){
                    this.storyBulkOperations.directMoveToProject(projectId, 
        ...
        _moveIntoArchive(story) {
            ...
            if(this.globalBacklogStories != null){
                this.removeStoryFromList(story, this.globalBacklogStories);
            }
        }
        ...
        _moveIntoGlobalBacklog(story) {
            // Just like _moveIntoArchive, but different
            if(this.globalBacklogStories != null){
                if (_.findWhere(this.globalBacklogStories, { id: story.id })) {
                    return;
                }
                this.globalBacklogStories.push(story);
                this.removeStoryFromList(story, this.boardProject.boardStories);
                this.removeStoryFromList(story, this.boardProject.archiveStories);
            }
        }
        ...
        _onSortSingleStory(storyId, item, parent, cellId, iterationId, placeholder, previousStory = null, projectId = null, releaseId = null) {
            ...
            */
            ...
            } else if (this.globalBacklogStories != null && iterationId === this.scope.$root['globalBacklogIteration'].id) {
                story.cell_id = null;
                story.release = releaseId == null ? null : {id: releaseId};
                this._moveIntoGlobalBacklog(story);
            } else {
