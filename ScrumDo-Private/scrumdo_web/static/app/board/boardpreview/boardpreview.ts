...
module scrumdo {
    ...
    export class BoardPreview {
        ...
        public renderDrawing = () => {
            ...
            this.collisionHelper = new scrumdo.BoardCollisionHelper(
                                this.scope.selectedCells,
                                this.scope);

            var cell = this.uiLayer.append("svg:rect").attr("class", "current-drawing"),
                newCell = this.uiLayer.append("svg:rect").attr("class", "current-drawing-header");

            cell.attr("x", sx).attr("y", sy).attr("width", ex - sx).attr("height", ey - sy);
            newCell.attr("x", sx).attr("y", sy).attr("width", ex - sx).attr("height", this.scope.gridsize);;

            var isTooltipVisible;
            var isCollide = this.collisionHelper.checkCollision(newCell[0][0]);

            if(isCollide){
                var tooltip = this.collisionHelper.tooltip();
                isTooltipVisible = true;
                cell.attr('class', 'current-drawing error');  
                tooltip.transition()        
                        .duration(200)        
                        .style("opacity", .9);        
