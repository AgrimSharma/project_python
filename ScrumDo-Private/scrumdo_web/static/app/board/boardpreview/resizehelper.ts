...
module scrumdo {
    export class BoardResizeHelper {
        ...
        public mode:any;
        private me:number;
        private collisionHelper: BoardCollisionHelper;
        ...
        public redraw() {
            ...
            this.collisionHelper = new scrumdo.BoardCollisionHelper(this.cells, this.scope);
            ...
            this.cells.forEach((cell) => {
                var layer = this.layer.append("svg:rect").attr("class", "resize-rect").attr("x", cell.gridsx() * this.gridsize).attr("y", cell.gridsy() * this.gridsize).attr("width", (cell.gridex() - cell.gridsx() + 1) * this.gridsize).attr("height", (cell.gridey() - cell.gridsy() + 1) * this.gridsize);

                if(this.mode != undefined) {
                    var validate = this.collisionHelper.validateCellCoordinates();
                    if(validate && !isTooltipVisible){
                        layer.attr('class', 'resize-rect error');
                        var tooltip = this.collisionHelper.tooltip();
                        isTooltipVisible = true;
                        tooltip.transition()        
                            .duration(200)        
                            .style("opacity", .9);        
                            tooltip.html('<b>Warning:</b> Some items have invalid coordinates. Unable to save this selection.');        
                    }

                    var isCollide = this.collisionHelper.checkCollision(cell);
                    if (isCollide) {
                        layer.attr(
                            'class', 'resize-rect error'
                        );

                        if(this.cells[0] && !isTooltipVisible) {
                            var tooltip = this.collisionHelper.tooltip();
                            isTooltipVisible = true;
                            tooltip.transition()        
                                    .duration(200)        
                                    .style("opacity", .9);        
                                    tooltip.html('<b>Warning:</b> Some items overlapped and may not render correctly.');
                        }
                    }
                }    

                layer.on("mousedown", this.beginMove);
                return layer;
            });
