...
module scrumdo {
    ...
    export class BoardEditController {
        ...
        constructor(public boardProject:BoardProject,
                    public scope,
                    public cellManager: BoardCellManager,
                    public headerManager: BoardHeadersManager,
                ...
                if(b.dirty || b == this.originalCell || b == this.originalHeader) {
                    if(b.header) {
                        ...
                        if(this.scope.editingCell && this.scope.editingCell.id == b.id) {
                            // Special case, the currently editing cell might have other properties changed
                            // that need to be copied in.
                            if(b.gridsy() >= 0 && b.gridsx() >= 0) {
                                // save cell if no negative coordinates
                                this.saveCell();
                            }
                        } else {
        ...
        public isSetupColumn(){
            return _.find(this.boardProject.boardCells, (cell: BoardCell) => cell.time_type == 1) != null;
        }
    }
}