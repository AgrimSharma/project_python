...
module scrumdo {
    export class SettingsController {
        ...
        archive() {
            if(this.projectData.currentProject.project_type == 2){
                this.alertService.warn("Archive Portfolio", "This is a Portfolio. Archiving this Portfolio \
                    will also archive all the Workspaces in this portfolio.").then(() => this.onConfirmArchive(2));
            }else{
        ...
        moveProject() {
            if(this.projectData.currentProject.project_type == 2){
                this.alertService.warn("Move Portfolio", "This is a Portfolio. Moving this Portfolio to another organization \
                    will also move all the Workspaces in this portfolio.").then(() => this.onConfirmMoveProject(2));
            }else{
        ...
        onConfirmArchive = (project_type: number = 1) => {
            let title: string = project_type == 1 ? 'Workspace' : 'Portfolio';
            this.confirmService.confirm(`Archive ${title}?`, `Are you sure you want to archive this ${title}?`, "No", "Yes", "secondary").then(() => {
                $("#archiveForm").submit();
            });
        }

        onConfirmMoveProject = (project_type: number = 1) => {
            let title: string = project_type == 1 ? 'Workspace' : 'Portfolio';
            this.confirmService.confirm(`Move ${title}?`, `Are you sure you want to move this ${title}?`, "No", "Yes", "secondary").then(() => {
                $("#moveForm").submit();
            });
        }
        ...
        deleteProject() {
            if(this.projectData.currentProject.project_type == 2){
                this.alertService.warn("Delete Portfolio", "This is a Portfolio. Deleting this Portfolio \
                    will also delete all the Workspaces in this portfolio.").then(() => this.onDeleteConfirm(2));
            }else{
                if(this.projectData.currentProject.portfolio_id != null){
        ...
        onDeleteConfirm = (project_type: number = 1) => {
            let title: string = project_type == 1 ? 'Workspace' : 'Portfolio';
            this.confirmService.confirm(`Delete ${title}?`, `Are you sure you want to delete this ${title}?`, "Cancel", `Delete ${title}`, "tertiary").then(() => {
                this.confirmDelete();
            });
        }
