...
module scrumdo {
    ...
    export class ProjectSettingsController {
        ...
        private continuousFlowIteration: Iteration;
        private showDestinationIteration: boolean = true;
        private backlogIteration: Iteration;
        private archiveIteration: Iteration;
        private destinationIteration: Iteration;
        private destinationIterationOption: Array<Iteration>;
        ...
        constructor(
            ...
            this.loadIterations();
            this.destinationIterationOption = [];
        }

        loadIterations(){
            this.iterations = _.filter(this.projectDatastore.iterations, (itr: Iteration) => itr.iteration_type == 1 && 
                                                                    !itr.hidden &&
                                                                    itr.end_date == null);
            this.backlogIteration = _.find(this.projectDatastore.iterations, (i) => i.iteration_type == 0);
            this.archiveIteration = _.find(this.projectDatastore.iterations, (i) => i.iteration_type == 2);
            if(this.project.continuous_flow){
                this.continuousFlowIteration = _.find(this.iterations, (itr: Iteration) => itr.id == this.project.continuous_flow_iteration_id)
            }

            this.scope.$watch("ctrl.continuousFlowIteration", this.onContinuousFlowUpdate);
            if(this.project.continuous_flow){
                this.showDestinationIteration = false;
            }
        }

        onContinuousFlowUpdate = () => {
            this.destinationIterationOption = [];
            if(this.continuousFlowIteration != null){
                this.destinationIterationOption.push(this.backlogIteration);
                this.destinationIterationOption.push(this.continuousFlowIteration);
                this.destinationIterationOption.push(this.archiveIteration);
                this.destinationIteration = this.continuousFlowIteration;
            }
        }
        ...
        save() {
            ...
            if(this.project.continuous_flow && this.continuousFlowIteration != null){
                this.project.continuous_flow_iteration_id = this.continuousFlowIteration.id;
                if(this.destinationIteration != null){
                    this.project.continuous_flow_destination_id = this.destinationIteration.id;
                }
            }else{
                this.project.continuous_flow_iteration_id = null;
            }
            let wasContinuous = this.scope.currentProject.continuous_flow;
            angular.copy(this.project, this.scope.currentProject);
            ...
            this.projectManager.saveProject(this.scope.currentProject).then((result) => {
                ...
                if(!result.continuous_flow){
                    if(wasContinuous){
                        this.projectDatastore.reloadIterations();
                    }
                    this.continuousFlowIteration = null;
                    this.project.continuous_flow = result.continuous_flow;
                    this.showDestinationIteration = true;
                    this.project.continuous_flow_destination_id = null;
                }else{
                    if(!wasContinuous){
                        this.projectDatastore.reloadIterations();
                    }
                ...
                if(result.prefix_error != null){
                    this.showToast(`${result.name} Prefix error.`);
                    ...
                    this.showToast(`${result.name} updated.`);
                }
        ...
        validateAutoArchiveDays(){
            if(this.project.auto_archive_iterations == null){
                this.project.auto_archive_iterations = 0;
            }else{
                this.project.auto_archive_iterations = parseInt(this.project.auto_archive_iterations)
            }
        }
    }
}
