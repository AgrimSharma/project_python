...
module scrumdo {
    export class SDDependenciesBoxController {
        ...
        select = ($event, option) => {
            ...
            this.isAdding = false;
            this.isLoading = true;
            if(this.scope.story.id == -1){
                this.scope.selected.push(option);
            }else{
        ...
        drawConnectingLines(){
            ...
            _.forEach(this.dependentOn, (s:any) => {
                data.push({ start: `#dependentOn-${s.id}`,
                            end: `#story-${this.scope.story.id}`,
                            orientation: 'depcurve',
                            stroke: '#f7b5a0'});
            });

            _.forEach(this.dependentTo, (s:any) => {
                data.push({ start: `#story-${this.scope.story.id}`,
                            end: `#dependentTo-${s.id}`,
                            orientation: 'depcurve',
                            stroke: '#f7b5a0' });
            });
