...
module scrumdo {
    export class FilterWidgetController {
        ...
        private selectedIterations: Array<Iteration>;
        ...
        constructor(
            ...
            if(this.scope.showIterations == null){
                this.scope.showIterations = false;
            }
            this.query = "";
            this.savedFilters = [];
            ...
            this.broadcastSignal = _.debounce(this._broadcastSignal, 200);

            // not to show for shared boards
            if(this.scope.$root['sharedBoard'] == null && this.scope.$root['sharedBoard'] !== true){
        ...
        dispatchFilter() {
                if (this.scope.projectSearch === 'true' || this.openNewTab) {
                    ...
                    if (this.scope.name === "backlogFilter") {
                        ...
                        if ((this.scope.$root.boardProject != null && this.scope.$root.boardProject.uiState.backlogOpen) 
                            || this.query !== "" 
                            || this.scope.$root.teamPlanningBacklog) {
                    ...
                    } else if (this.scope.name === "globalBacklogFilter") {
                        this.scope.$emit('globalBacklogFilterChange', this.query, this.scope.name);
                        if ((this.scope.$root.boardProject != null && this.scope.$root.boardProject.uiState.loadGlobalBacklog) 
                            || this.query !== "" 
                            || this.scope.$root.teamPlanningBacklog) {
                            this.scope.$emit('filter', this.query, this.scope.name);
                            try {
        ...
        openFilterBuilder(title = 'Filter Builder') {
            ...
            this.savedSearchDialog = this.modal.open({
                ...
                resolve: {
                    ...
                    cells: () => cells,
                    showIterations: () => this.scope.showIterations
                }
            });
        ...
        _broadcastSignal = () => {
            this.scope.$emit("filterValueChanged", {query: this.query});
        }
    }
}