...
module scrumdo {
    export class StoryManager {
        ...
        constructor(
            ...
            this.Story = <StoryResource>this.resource(API_PREFIX + "organizations/:organizationSlug/projects/:projectSlug/iterations/:iterationId/stories/:id",
                ...
                },
                {
                    ...
                    byProjectMiniStories: {
                        method: 'GET',
                        url: API_PREFIX + "organizations/:organizationSlug/projects/:projectSlug/ministories",
                        isArray: true
                    },
        ...
        private addStoryToIterationRelease(story) {
            ...
            if ( (story.iteration_id in this.iterationReleaseMap) &&
                story.release == null &&
                (-1 in this.iterationReleaseMap[story.iteration_id] )  ) {
                this._addNoDuplicates(story, this.iterationReleaseMap[story.iteration_id][-1]);
            }
        }
        ...
        onAddedStoryLoaded = (story) => {
            ...
            if(this.storiesByProject[story.project_slug] != null){
                this._addNoDuplicates(story, this.storiesByProject[story.project_slug]);
            }
        ...
        onDel = (event, message) => {
            ...
            var storyId = message.payload.id;
            var projectSlug = message.payload.project_slug;
            trace("Remote delete story " + storyId + " from project " + projectSlug);
            this.onRemoved(storyId, projectSlug);
        ...
        convertEpic(story) {
            ...
            p.then((result) => {
                ...
                // send signal to refresh dashboard stories
                this.sendSignalforDashboardStories();
            });
            return p;
        }
        ...
        duplicate(story) {
            ...
            p.then((newStory) => {
                ...
                if(this.storiesByProject[newStory.project_slug] != null){
                    this.storiesByProject[story.project_slug].push(newStory);
                }
        ...
        loadActiveStories() {
            ...
            p.then((models) => {
                ...
                this.dashboardStoriesLoaded = true;
            });
        ...
        loadStoriesByReleaseId(portfolioSlug: string, releaseID: number, type: string = null, active_only = null) {
            if(type == "mini"){
                var p = this.Story.miniStoriesByReleaseId({ organizationSlug: this.organizationSlug, 
                                                            projectSlug: portfolioSlug, releaseID: releaseID, 
                                                            active_only:active_only }).$promise;
            }else{
        ...
        searchProject(projectSlug: string, query: string, page: number = 1, 
                      search_backlog: boolean = true, 
                      search_archive: boolean=true) {
            var p = this.Story.byProjectWithFilter({ organizationSlug: this.organizationSlug, 
                                                    projectSlug: projectSlug, 
                                                    query: query, 
        ...
        loadProjectStories(projectSlug: string){
            var p = this.Story.byProjectMiniStories({ organizationSlug: this.organizationSlug, projectSlug: projectSlug}).$promise;
            p.then(this.trackStories);
            p.then((stories) => {
                this.storiesByProject[projectSlug] = stories;
            });
            return p;
        }
        ...
        moveToProject(story, projectSlug: string, iterationId: number, cellId: number = null, release_id: number = null) {
            var url = API_PREFIX +
            ...
            p.then((newStory: any) => {
                ...
                story.prefix = newStory.data.prefix;
                story.global_backlog_card = newStory.data.global_backlog_card;
                story.number = newStory.data.number;
                story.iteration_name = newStory.data.iteration_name;
                story.epic = newStory.data.epic;
                story.release = newStory.data.release;
        ...
        deleteStory(story) {
            ...
            let p = story.$remove();
            p.then(this.sendSignalforDashboardStories);
            return p;
        }

        sendSignalforDashboardStories = () => {
            if(this.dashboardStoriesLoaded){
                this.rootScope.$broadcast('reloadDashboardMyStories', null);
            }
        }

        onRemoved = (storyId, projectSlug: string = null) => {
            ...
            }else if(story.project_slug == projectSlug){
                this.removeReferences(story, story.id);
            }
        ...
        removeFromEpic(story, epicId){
            var ref = this.storiesByEpic,
                list, i;
            for (epicId in ref) {
                list = ref[epicId];
