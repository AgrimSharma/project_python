...
storymodule.directive("sdDragOver", function(){
   return {
       restrict: 'A',
       link: function(scope, element, attrs) {
           element.bind('dragenter', function(event:JQueryEventObject){
                var dragEvent = <DragEvent>event.originalEvent;
                var dataTransfer = dragEvent.dataTransfer;
                if(dataTransfer.effectAllowed != 'all') return;
                element.addClass('isActive-dropzone');
