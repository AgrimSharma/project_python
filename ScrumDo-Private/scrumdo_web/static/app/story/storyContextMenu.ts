...
module scrumdo {
    export class storyContextMenu {
        ...
        constructor(private scope,
            ...
            this.$win = $(window);
            this.$dom = $(document);
            this.$board = $('.scrumdo-boards-wrapper, .planning-column-content, .scrumdo-backlog-wrapper, .card-grid-container, \
                            .inbox-group, .scrumdo-wrapper,div[class$=\'-wrapper\'] ');
            ...
            element.on('contextmenu.dirContextMenu', (e) => {
                ...
                if (scope.$root.selectedCount > 1 && scope.$root.ctxmenus.length > 2) {
                    extraMenu = [
                        ...
                        { label: `Move ${this.workItemNameP} to Workspace/Iteration`, action: 'moveCardsToProject', class: 'multiple' },
