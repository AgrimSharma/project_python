...
module scrumdo {
    export class PlanningController {
        ...
        // Temporarily hide the story map feature.
        private showStoryMapping: boolean = false;
        private timePeroidName: string;
        private saveQueue;
        private saveQueueDup;
        ...
        constructor(
            ...
            this.scope.$on('bulkOperationFinished', this.onBulkOperationFinished);
       ...
       initData(){
           ...
           this.timePeroidName = this.scope.$root['safeTerms'].current.time_period_name;
       ...
       initLocalStorage(){
        ...
        addIteration() {
            return this.iterationWindowService.createIteration(this.organizationSlug, this.projectSlug, {}, this.timePeroidName);
        }
        ...
        onSortStory = (event) => {
            ...
            var fromEpicId = parseInt(item.parent().attr("data-epic-id"));
            fromEpicId = isNaN(fromEpicId) ? null : fromEpicId;
            releaseId = isNaN(releaseId) ? null : releaseId;

            if(toContainerType == "safe-sibling-container"){
                trace("Card dropped to Sibling Project Column");
                if(!this.userService.canWrite(projectId)){
                    this.alertService.alert(`Access Error`, 
                            `You don't have write access to this Workspace.`);
                    return;
                }
            }
            if(toContainerType == "epic-story-list" && iterationId <= 0 
                && (fromContainerType == "safe-backlog-container" || fromContainerType == "safe-sibling-container")){
                let name = this.scope.$root['safeTerms'].current.time_period_name;
                this.alertService.alert(`${name} Error`, 
                    `Please first select ${name} to move card to specific ${name}\'s Collection.`);
                return;
            }
            var stories, story, storyId;
            if (this.getSelectedCount() === 0) {
                storyId = parseInt(item.attr("data-story-id"));
                story = this._onSortSingleStory(storyId, item, parent, toContainerType, placeholder, null, fromContainerType);
                stories = [story];
            } else {
                ...
                for (var i = 0, len = stories.length; i < len; i++) {
                    var story = stories[i];
                    this._onSortSingleStory(story.id, item, parent, toContainerType, placeholder, previousStory, fromContainerType);
                    previousStory = story;
                }
            }

            if(toContainerType == "safe-backlog-container"){
                trace("Card dropped to Global Backlog Column");
                if(projectId != null && projectId !== story.project_slug){
                    this.storyBulkOperations.directMoveToProject(projectId, 
                                            stories[0].iteration_id, 
                                            stories, 
                                            null, 
                                            releaseId, 
                                            {type: "removeFromEpic", metaData: {"epicId": fromEpicId}});
                    placeholder.remove();
                    this.scope.$apply();
                    return;
                }
            ...
            if(fromContainerType == "safe-backlog-container" || fromContainerType == "safe-sibling-container"){
                trace("Card dragged from Global Backlog or Sibling Project Column");
                if(toContainerType == "iteration-story-list"){
                    trace("Card dropped to Iteration Column");
                    this.storyBulkOperations.directMoveToProject(this.projectSlug, 
                                            stories[0].iteration_id, 
                                            stories);
                    placeholder.remove();
                    this.scope.$apply();
                ...
                if(toContainerType == "epic-story-list"){
                    trace("Card dropped to Epic Column");
                    if(iterationId > 0){
                        this.storyBulkOperations.directMoveToProject(this.projectSlug, 
                                            stories[0].iteration_id, 
                                            stories, 
                                            null,
                                            null,
                                            {type: "moveToEpic", metaData: {"epicId": epicId}});
                    }else{
                        let name = this.scope.$root['safeTerms'].current.time_period_name;
                        this.alertService.alert(`${name} Error`, 
                            `Please first select ${name} to move card to specific ${name}\'s Collection.`);
        ...
        onBulkOperationFinished = (event, data: {type: string, metaData:any, data: Array<Story>}) => {
            if(data.type != null && data.type == 'removeFromEpic' && data.metaData["epicId"] != null){
                _.forEach(data.data, (story) => {
                    this.storyManager.removeFromEpic(story, data.metaData["epicId"]);
                });
            }
            if(data.type != null && data.type == 'moveToEpic'){
                var epicId = data.metaData["epicId"];
                var stories = [];
                epicId = epicId > 0 ? epicId : null;
                _.forEach(data.data, (story:any) => {
                    story.epic = {id: epicId};
                    stories.push(story);
                });
                this.storyManager.bulkSave(data.data);
            }
        }

        bulkMoveToProject(projectSlug: string, iterationId: number, stories: Array<Story>, cellId: number = null, updateStories = false){
            this.saveQueue = stories;
            this.saveQueueDup = [];
            this.onMultiMoveProjectConfirm(projectSlug, iterationId, cellId, updateStories);
        }

        onMultiMoveProjectConfirm = (projectSlug, iterationId, cellId = null, updateStories = false) => {
            this.moveNextInQueueToProject(projectSlug, iterationId, cellId, updateStories);
        }

        moveNextInQueueToProject = (projectSlug, iterationId, cellId = null, updateStories = false) : ng.IPromise<any> => {
            var story = this.saveQueue.shift();
            if (typeof story === "undefined" || story === null) {
                if(updateStories){
                    this.storyManager.bulkSave(this.saveQueueDup);
                }
                return;
            }
            this.storyManager.moveToProject(story, projectSlug, iterationId, cellId).then((data:any) => {
                        var s: Story = data.data;
                        s.release = story.release;
                        s.epic = story.epic;
                        this.saveQueueDup.push(s);
                        this.moveNextInQueueToProject(projectSlug, iterationId, cellId, updateStories);
            });
        }
        ...
        _onSortSingleStory(storyId, item, parent, dragType, placeholder, previousStory = null, fromContainerType = null) {
            ...
            if (parent.attr("data-epic-id")) {
                var iterationId = parseInt(parent.attr("data-iteration-id"));
                if((fromContainerType == "safe-backlog-container" && iterationId > 0) || fromContainerType != "safe-backlog-container"){
                    var epicId = parseInt(parent.attr("data-epic-id"));
