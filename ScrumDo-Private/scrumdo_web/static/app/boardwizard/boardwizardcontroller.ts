    ...
    export class BoardWizardController {
        ...
        reset() {
            ...
            if ((this.project != null) && this.project.project_type === 2) {
                this.steps = [
                    { 'name': 'Idea Generation', 'color': "#747E89", 'style': 1, 'sub': ['Input Queue', 'Waiting for Validation'], 
                        'policy_text': 'At this stage Portfolio options are generated.', 'time_type': 1 },
                    { 'name': 'Idea Validation', 'color': "#F5764E", 'style': 1, 'sub': ['Doing', 'Waiting for Approval'], 
                        'policy_text': 'Analysis and Experimentation are taken up to understand whether or not to continue.', 'time_type': 2},
                    { 'name': 'Committed', 'color': "#6c8fb4", 'style': 8, 'sub': ['Discarded'] ,
                        'policy_text': 'A commitment is made to implement the idea.', 'time_type': 1},
                    { 'name': 'Idea Implementation', 'color': "#3898DB", 'style': 1, 'sub': ['Doing', 'Waiting for Realization'] ,
                        'policy_text': 'Building and Implementation of the idea.', 'time_type': 2, 'sub_layout': [9, 0]},
                    { 'name': 'Value Realization', 'color': "#34CC73", 'style': 0, 'sub': ['Doing', 'Done'] ,
                        'policy_text': 'Realizing value in the market place.'},
                    { 'name': '', 'color': "#9A59B5", 'style': 0, 'sub': ['Doing', 'Done'] }
                ];
                this.rows = [
                    { name: '' }
                ];
            } else if((this.project != null) && this.project.children_count > 0){
                this.steps = [
                    { 'name': 'Funnel', 'color': "#747E89", 'style': 0, 'sub': ['Doing', 'Done'], 'time_type': 1 },
                    { 'name': 'Analysis', 'color': "#3898DB", 'style': 9, 'sub': ['Doing', 'Done'], 'time_type': 2 },
                    { 'name': 'Implementation', 'color': "#34CC73", 'style': 9, 'sub': ['Doing', 'Done'], 'time_type': 2 },
                    { 'name': 'Qualifying', 'color': "#F5764E", 'style': 9, 'sub': ['Doing', 'Done'], 'time_type': 2 },
                    { 'name': 'Done', 'color': "#747E89", 'style': 0, 'sub': ['Doing', 'Done'] },
                    { 'name': '', 'color': "#9A59B5", 'style': 0, 'sub': ['Doing', 'Done'] }
                ];
                this.rows = [
                    { name: '' }
                ...
                this.steps = [
                    { 'name': 'Todo', 'color': "#747E89", 'style': 0, 'sub': ['Doing', 'Done'] , 'time_type': 1},
                    { 'name': 'Doing', 'color': "#3898DB", 'style': 1, 'sub': ['In Progress', 'Waiting for Review'] },
