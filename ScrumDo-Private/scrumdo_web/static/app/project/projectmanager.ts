...
module scrumdo {
    ...
    export interface IProject {
        ...
        color: number;
        icon: string;
        children: Array<MiniParentProject>;
        continuous_flow: boolean;
        continuous_flow_iteration_id: number;
        vision: string;
    }
    ...
    export class ProjectManager {
        ...
        public dashboardProjects = {};

        private projectApi:ProjectResource;
        private DashProjectsApi: ng.resource.IResourceClass<Project>;
        ...
        constructor(public resource:ng.resource.IResourceService,
            ...
            this.DashProjectsApi = <ProjectResource> this.resource(API_PREFIX + "organizations/:organizationSlug/dashprojects",
                {
                organizationSlug: this.organizationSlug    
                },
                {
            ...
            this.projectApi = <ProjectResource> this.resource(API_PREFIX + "organizations/:organizationSlug/projects/:projectSlug",
                ...
                },
                {
                    ...
                    loadKanbanStats: {
                        method: "GET",
                        url: API_PREFIX + "organizations/:organizationSlug/projects/:projectSlug/kanbanstats",
                        params: {
                            organizationSlug: this.organizationSlug,
                            projectSlug: "projectSlug"
                        }
                    ...
                    loadDependencies: {
                        method: "GET",
                        url: API_PREFIX + "organizations/:organizationSlug/projects/:projectSlug/dependencies",
                        isArray: false,
                        params: {
                            organizationSlug: this.organizationSlug,
                            projectSlug: "projectSlug"
                    ...
                    loadTeamMembers: {
                        method: "GET",
                        url: API_PREFIX + "organizations/:organizationSlug/projects/:projectSlug/team",
                        isArray: true,
                        params: {
                            organizationSlug: this.organizationSlug,
                            projectSlug: "projectSlug"
        ...
        public loadDashBoardProjects(organizationSlug, stats:any = false, reload:boolean = false){
            var deferred, p;
            if (organizationSlug in this.dashboardProjects && !reload) {
                deferred = this.$q.defer();
                deferred.resolve(this.dashboardProjects[organizationSlug]);
                return deferred.promise;
            } else {
                p = this.DashProjectsApi.query({
                    organizationSlug: organizationSlug,
                    stats: stats
                }).$promise;
                p.then((projects) => {
                    this.dashboardProjects[organizationSlug] = projects;
                });
            }
            return p;
        }
        ...
        public loadKanbanStats(projectSlug:string){
            var p = this.projectApi.loadKanbanStats({projectSlug:projectSlug}).$promise;
            return p;
        }

        public loadDependency(projectSlug:string, stats: boolean = true, sibling: boolean = true){
            var p = this.projectApi.loadDependencies({projectSlug:projectSlug, stats: stats, sibling: sibling}).$promise;
            return p;
        }

        public loadTeamMembers(projectSlug: string){
            var p = this.projectApi.loadTeamMembers({projectSlug:projectSlug}).$promise;
            return p;
        }
    }
};
