    ...
    export class ProjectAppController {
        ...
        constructor(private $scope:ng.IScope,
            ...
            if(uiTabs[view]!=null && uiTabs[view] === false){
                trace('Attempted to load a project tab that we should not have in this project' + $state.current.name);
            ...
            this.$scope.$root.$on('$stateChangeStart', (event, toState, toParams, fromState, fromParams) => {
                trace(`$stateChangeStart: from ${fromState.name} to ${toState.name}`);
                trace(toParams);
            });

            this.$scope.$root.$on('$stateChangeError', (event, toState, toParams, fromState, fromParams) => {
                trace(`$stateChangeError: from ${fromState.name} to ${toState.name}`);
                trace(toParams);
            });

            this.$scope.$root.$on('$stateChangeSuccess', (event, toState, toParams, fromState, fromParams) => {
                trace(`$stateChangeSuccess: from ${fromState.name} to ${toState.name}`);
                trace(toParams);
            });

            this.$scope.$root.$on('$stateChangeSuccess', (event, toState, toParams, fromState, fromParams) => {
               if( ($state.current.name != 'app.iteration.cards' && $state.current.name != 'app.iteration.activity') &&
