            ...
            controller: ["$scope", "$timeout", "$attrs", function($scope, $timeout, $attrs) {
                ...
                $document.on(
                    'keydown keypress paste', function (event) {
                        ...
                        // prevent menu-item pop-up again after selecting the user
                        //activate again on @ keypress event
                        if (!activeMenuScope) {
                            if(event.shiftKey && event.which === 64 && $scope.$root.pauseMentiofn == true){
                                $scope.$root.pauseMentiofn = false;
                            }
                        }
                        if (activeMenuScope) {
                            if (event.which === 13) {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.$watch(
                    ...
                    function (newValue) {
                        ...
                        //prevent menu pop-up to show just after seleting the item
                        if (scope.$root.pauseMentiofn){
                            return;
                        }
                            ...
                            )
                        {
                            ...
                            //trigger resize event to reposition the menu
                ...
                scope.$watch('items', function (items) {
                    if (items && items.length > 0) {
                        ...
                        if (!scope.visible && scope.requestVisiblePendingSearch) {
                            ...
                            //set menuItem scroll back to top if not
                            mentioUtil.setMenuScrollToTop();
                        }
                    } else {
                        if(scope.hideMenu) {
                            scope.hideMenu();
...
angular.module('mentio')
    .factory('mentioUtil', ["$window", "$location", "$anchorScroll", "$timeout", function ($window, $location, $anchorScroll, $timeout) {
        ...
        // public
        function setMenuScrollToTop(){
            setTimeout(function(){
               $('.sdMentio:visible ul.user-search').scrollTop(0);
            },200);
        }
        ...
        function localToGlobalCoordinates(ctx, element, coordinates) {
            ...
            //reposition menu if not enough space botttom of window
            setTimeout(function(){
                setPosition();
            },10);
         }
        ...
        function setPosition(){
            if($('.sdMentio:visible').length>0){
                var top_pos = $('.sdMentio:visible').offset().top;
                var left_pos = $('.sdMentio:visible').offset().left;
                var window_height = window.innerHeight;
                var menuHeight = $('.sdMentio:visible').outerHeight();
                if((window_height - top_pos ) < menuHeight){
                    top_pos -= (menuHeight+10)
                    $('.sdMentio:visible').offset({ top: top_pos, left: left_pos});
                }
            }
        }
        ...
        return {
            ...
            setMenuScrollToTop: setMenuScrollToTop,
