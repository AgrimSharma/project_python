...
class IfNotSettingNode(template.Node):
    def __init__(self, nodelist_true, nodelist_false, setting):
        self.nodelist_true = nodelist_true
        self.nodelist_false = nodelist_false
        self.setting = setting
    ...
    def render(self, context):
        try:
            ...
            # attribute didn't exist, so assume false.            
            setting_value = False
...
def ifnotsetting(parser, token):
    bits = token.split_contents()
    nodelist_true = parser.parse(("else", "endifnotsetting"))
    token = parser.next_token()
    if token.contents == "else":
        nodelist_false = parser.parse(("endifnotsetting",))
        parser.delete_first_token()
    else:
        nodelist_false = template.NodeList()
    return IfNotSettingNode(nodelist_true, nodelist_false, bits[1])