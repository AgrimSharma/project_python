...
import apps.debug_toolbar.urls
from apps.debug_toolbar.toolbar.loader import DebugToolbar
    ...
    def _show_toolbar(self, request):
        ...
        if not remote_addr in settings.INTERNAL_IPS \
            or (request.is_ajax() and \
                not apps.debug_toolbar.urls._PREFIX in request.path) \
    ...
    def process_request(self, request):
        if self.show_toolbar(request):
            if self.override_url:
                ...
                apps.debug_toolbar.urls.urlpatterns += patterns('',
