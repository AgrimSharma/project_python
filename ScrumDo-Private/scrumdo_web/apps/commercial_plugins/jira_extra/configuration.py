...
def doProjectConfiguration(extra, request, project, stage=""):
    ...
    if request.GET.get("syncronize") == "now":    
        # logger.debug("Setting to sync")
        configuration[STATUS_FIELD] = "Synchronizing"
        extra.manager.queueSyncAction( extra.getSlug(), project, SyncronizationQueue.ACTION_SYNC_REMOTE )
...
def doFilterConfiguration(extra, request, project, configuration):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            configuration[IMPORT_FIELD] = form.cleaned_data["importStories"]
            configuration[FILTER_FIELD] = form.cleaned_data["jiraFilter"]
            configuration[PROJECT_FIELD] = form.cleaned_data["project"]
            
            configuration[FILTER_NAME_FIELD] = [choice[1] for choice in choices if choice[0] == form.cleaned_data["jiraFilter"] ][0]
            extra.saveConfiguration( project.slug, configuration )
            # if configuration[IMPORT_FIELD]:
            #     return HttpResponseRedirect( reverse('configure_extra_with_stage', args=[extra.getSlug(), project.slug, "statuses"]) )
            # else:
            #     # If we're not importing bugs as stories, we don't need to worry about status configuration
            return HttpResponseRedirect( reverse('configure_extra_with_stage', args=[extra.getSlug(), project.slug, "summary"]) )            
    else:
