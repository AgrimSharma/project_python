...
from apps.django_openid.models import DjangoOpenIDStore
from apps.django_openid.utils import OpenID, Router
from apps.django_openid import signed
from apps.django_openid.response import TemplateResponse
...
Fzk0lpcjIQA7""".strip()
    ...
    def do_logo(self, request):
        return HttpResponse(
            self.OPENID_LOGO_BASE_64.decode('base64'), content_type='image/gif'
        )
...
class SessionConsumer(LoginConsumer):
    ...
    def __init__(self):
        super(SessionConsumer, self).__init__(SessionPersist)
