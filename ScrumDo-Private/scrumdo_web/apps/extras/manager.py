...
import apps.projects.limits as project_limits
import apps.extras.signals as extras_signals

from models import ProjectExtraMapping, SyncronizationQueue, ExternalStoryMapping, StoryQueue, ExternalTaskMapping
...
class ExtrasManager:
    ...
    def queueSyncAction( self, extra_slug, project, action, **kwargs):
        ...
        if kwargs.get("external_id"):
            queueObject.external_id = kwargs.get("external_id")
        elif queueObject.task == None:
            try:
    ...
    def getExtraConfigs( self , project):
        ...
        rv = []
        for extra in extras:
            if not extra.availableFor(project):
                continue
            ...
            if extra.isPremium() and project.organization!=None and not project_limits.org_extra_limit.increaseAllowed(organization=project.organization):
                continue
            config = {}
            if hasattr(extra,"DEPRECATED"):
                if extra.DEPRECATED and not self.is_extra_enabled(project, extra.getSlug()):
                    continue
                config["DEPRECATED"] = extra.DEPRECATED
            config["extra"] = extra
    ...
    def enableExtra( self, project, extra_slug ):
        if not self.is_extra_enabled(project, extra_slug ):
    ...
    def onNewsFeed(self, sender, news_item, *args, **kwargs):
        self.queueSyncActions( news_item.project, SyncronizationQueue.ACTION_NEWS_POSTED, external_id=news_item.id)
    ...
    def __init__(self, extras_settings):
        ...
        project_signals.news_posted.connect(self.onNewsFeed, dispatch_uid="extra_signal_hookup")
