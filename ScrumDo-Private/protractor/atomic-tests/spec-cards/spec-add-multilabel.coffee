...
describe "Scrumdo card multilabel", ->
    ...
    it "Should add card to cell with a story labels", ->
        browser.get param.projectUrl
        ...
        element.all(By.css('.kanban-cell .scrumdo-column-title .scrumdo-column-dropdown')).get(0).element(By.tagName('button')).click().then ->
            element.all(By.css('.scrumdo-column-title')).get(0).all(By.css('.dropdown-menu li a')).get(0).click().then ->
                element(By.css('#summaryEditor div.scrumdo-mce-editor')).sendKeys(cardName).then ->
                    element.all(By.css('.scrumdo-btn.dropdown-main-button.primary.extended.scrumdo-select-button')).get(0).click().then -> element.all(By.css('.labels')).get(2).click().then ->
    ...
    cardName = param.cardName
    it "Should add card to cell with a all labels", ->
        browser.get param.projectUrl
        
        element.all(By.css('.kanban-cell .scrumdo-column-title .scrumdo-column-dropdown')).get(0).element(By.tagName('button')).click().then ->
            element.all(By.css('.scrumdo-column-title')).get(0).all(By.css('.dropdown-menu li a')).get(0).click().then ->
                element(By.css('#summaryEditor div.scrumdo-mce-editor')).sendKeys(cardName).then ->
                    element.all(By.css('.scrumdo-btn.dropdown-main-button.primary.extended.scrumdo-select-button')).get(0).click().then -> element(By.xpath('//span[contains(text(),"Bug")]')).click().then -> element(By.xpath('//span[contains(text(),"Feature")]')).click().then -> element(By.xpath('//span[contains(text(),"User Story")]')).click().then ->
