...
class GenerateOctroiPdf(GeneratePdf):
    ...
    def generate_pdf_by_ptype(self, ptype):
        print '2.',ptype
        ships_count = self.billing.shipments.filter(shipment__product_type=ptype).count()
    ...
    def get_awb_table_data(self, ptype, index):
        awb_shipments = self.shipments.filter(shipment__product_type=ptype)[index-1:index-1+self.PDF_CHUNK_SIZE]
        ...
        for shipment in awb_shipments:
            ...
            yield [current_index,
                 ...
                 shipment.octroi_charge,
                 shipment.octroi_ecom_charge,
                 total]
    ...
    def generate_awb_pdf_chunk(self, index, ptype, add_total):
        ...
        awb_table_headers = [
            ...
            self.awbpara('Declared Value', "AWB_heading"),
            self.awbpara('Octroi Charge', "AWB_heading"),
            self.awbpara('Ecom Charge', "AWB_heading"),
...
class GenerateOctroiExcel(GenerateReport):
    ...
    def get_awb_table_data_direct(self, ptype):
        shipments = self.billing.shipments.filter(shipment__product_type=ptype)
    ...
    def generate_awb_excel(self):
        ...
        # COD SECTION ####################
        cod_shipments = self.billing.shipments.filter(shipment__product_type='cod').exists()
        if cod_shipments:
