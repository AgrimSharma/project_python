...
def get_awb_details(request):
   if request.POST:
     ...
     for a in awb:
       ...
       if a in abc[0].airwaybill_number.all():
           chck=True
       else:
           chck=False
       ...
       if ship and chck:
          ...
          frt_status="Y"
          ref_pickup=request.POST['manifest']
     ...
     return HttpResponseRedirect("mobile/no_manifest_ships.html",{"ships":ships},context_instance=RequestContext(request))
   else:
