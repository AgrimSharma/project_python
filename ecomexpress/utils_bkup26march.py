...
def get_zone(customer, city):
    if not customer.zone_label:
        return city.zone

    zone = Zone.objects.filter(label_city=city, label=customer.zone_label)
    if zone:
        return zone[0]
    else:
        return city.zone
...
def shipment_mail(shipment):
         ...
         to_email = ("samar@prtouch.com","sravank@ecomexpress.in")
         email_msg = subject
...
#
     ...
     org_zone = get_zone(shipment.shipper, org_city)
     if shipment.original_dest: 
         ...
         dest_zone = get_zone(shipment.shipper, dest_city)
...
def history_update(shipment, status, request, remarks="", reason_code=None):
...
#    if shipment.reason_code:
 #       reason_code = shipment.reason_code
 #   else:
  #      reason_code = None
    status = status
    remarks = remarks
    ...
    shipment_history.objects.create(shipment=shipment, status=status, employee_code = employee_code, current_sc = current_sc, expected_dod=shipment.expected_dod, reason_code=reason_code, remarks=remarks)
    ShipmentExtension.objects.filter(shipment_id=shipment.id).update(status_bk = status, current_sc_bk = current_sc, remarks = remarks, updated_on = now)
...
def admin_only(function):
    ...
    def _inner(request, *args, **kwargs):
        if not request.user.employeemaster.employee_code == '124':
...
def admin_and_others(function):
    def _inner(request, *args, **kwargs):
        if not request.user.employeemaster.employee_code in ['124','217','119']:
              raise PermissionDenied
        return function(request, *args, **kwargs)
    return _inner
...
def shipment_mail(shipment):
         subject = "Shipment: %s updated with %s code"%(shipment.airwaybill_number, shipment.reason_code)
         from_email = "support@ecomexpress.in"
         to_email = ("onkar@prtouch.com","sravank@ecomexpress.in", "sunily@ecomexpress.in", "pawant@ecomexpress.in", "mohinderk@ecomexpress.in", "pravinp@ecomexpress.in", "sandeepv@ecomexpress.in")
         email_msg = subject
         send_mail(subject,email_msg,from_email,to_email)
