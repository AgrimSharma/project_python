...
def get_dc_address(consignee_address1, consignee_address2, consignee_address3, destination_city, state):
    if consignee_address1:
        address1 = consignee_address1 
    if consignee_address2:
        address2 = consignee_address2 
    if consignee_address3:
        address3 = consignee_address3 
    if destination_city: 
    ...
    ship_addr = address2 + address3+','+city+',' + state
    #print "Ship addr: ", awb, '\t', ship_addr
...
def get_shipment_address(awb):
    ...
    ship_addr = address2 + address3+','+address4+','+city+',' + state
    #print "Ship addr: ", awb, '\t', ship_addr

    return ship_addr

def get_display_address(awb):
    ship = Shipment.objects.get(airwaybill_number=awb)
    address1, address2, address3, address4, state, city = '', '', '', '', '', ''
    if ship.consignee_address1:
        address1 = ship.consignee_address1 
    ...
    address = [address1, address2, address3, address4, city, state]
    clean_address = [x.strip() for x in address if x.strip()]
    display_add = ', '.join(clean_address) + '\n Pincode: ' + str(ship.pincode)
...
def update_dc_lat_lng():
    ...
    W = px.load_workbook('/tmp/dc_address.xlsx', use_iterators = True)
...
def update_add_coords(file_path):
    '''
    Incrementally update Add_Coords from a xlsx/csv file.
    '''
    import openpyxl as px

    W = px.load_workbook(file_path, use_iterators = True)
    p = W.get_sheet_names()[0]
    ws = W.get_sheet_by_name(p)
    data = [[cell.value for cell in row ] for row in ws.iter_rows()]
    for row in data:
        print row
	try:
            dc = ServiceCenter.objects.get(center_shortcode=row[0])
