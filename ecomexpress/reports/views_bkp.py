...
@json_view
def get_origin_state_details(request):
    if request.method == 'GET':
        origin_state = request.GET.get('origin_state')
        origin_sc = ServiceCenter.objects.filter(city__state_id__in=origin_state)
    ...
    if request.POST:
       ...
       # shipments = query(customer, date_from, date_to, sc)
        ...
        if report_type == "dl":
             ...
             sheet.write(3, 5, "Origin Region", style=header_style)
             sheet.write(3, 6, "Origin State", style=header_style)
             sheet.write(3, 7, "Shipper", style=header_style)
             sheet.write(3, 8, "Consignee", style=header_style)
             sheet.write(3, 9, "Collectable Value", style=header_style)
             sheet.write(3, 10, "Status Updated On", style=header_style)
             sheet.write(3, 11, "Mobile Number", style=header_style)
             sheet.write(3, 12, "Reason Code", style=header_style)
             sheet.write(3, 13, "Reason Description", style=header_style)
             sheet.write(3, 14, "Remarks", style=header_style)
             sheet.write(3, 15, "Number of attempts", style=header_style)
             sheet.write(3, 16, "Telecalling Comment", style=header_style) 
             sheet.write(3, 17, "Current Service Centre", style=header_style) 
             sheet.write(3
            ...
            return render_to_response('reports/shipment_reports_filter.html',
                                  {'shipments':download_list,'ships':shipments,'qq':q},
                                  context_instance=RequestContext(request))

    else:
        customer=Customer.objects.using('local_ecomm').all()
        sc = ServiceCenter.objects.using('local_ecomm').all()
        region = Region.objects.using('local_ecomm').all()
        state = State.objects.using('local_ecomm').all()
        return render_to_response("reports/NDR.html",
                              {'customer':customer,
                               'sc':sc, 'region': region, 'state': state},
                               context_instance=RequestContext(request))
