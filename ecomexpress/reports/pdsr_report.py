...
def get_pdsr_dispaly_data(date_str):
    ...
    day_data = Shipment.objects.using('local_ecomm').filter(shipment_date=date_str).\
            exclude(shipext__product__in=[ebsppd, ebscod])\
    ...
    ddata = Shipment.objects.using('local_ecomm').filter(shipment_date=date_str).\
            exclude(shipext__product__in=[ebsppd, ebscod]).values("shipper__code", "shipper__name", "product_type").\
            annotate(
                ship_count=Count('id'),
                total_cw=Sum('chargeable_weight'),
                freight_sum=Sum('order_price__freight_charge'),
    ...
    #print ddata
    #print "product wise data"
    day_cod = Shipment.objects.using('local_ecomm').filter(shipment_date=date_str).\
        exclude(shipext__product__in=[ebsppd, ebscod]).exclude(rts_status = 1).values("shipper__code").\
    ...
    month_data = Shipment.objects.using('local_ecomm').filter(shipment_date__year=year,
            shipment_date__month=month, shipment_date__lte=date_str)\
            .exclude(shipext__product__in=[ebsppd, ebscod])\
    ...
    mdata = Shipment.objects.using('local_ecomm').filter(shipment_date__year=year,
            shipment_date__month=month, shipment_date__lte=date_str).\
            exclude(shipext__product__in=[ebsppd, ebscod]).\
    ...
    month_cod_cod = Shipment.objects.using('local_ecomm').filter(shipment_date__year=year, shipment_date__month=month,
    ...
    for d in day_product_wise:
        ...
        if ptype == 'cod':
            ...
            if day_product_wise_dict[d['shipper__code']].get('ppd'): day_product_wise_dict[d['shipper__code']]['ppd'] = int(day_product_wise_dict[d['shipper__code']]['ppd']) + int(d)
            else: day_product_wise_dict[d['shipper__code']]['ppd'] = d
    ...
    def get_prodwise_row(day_data, month_data, day_cod_sum=0, month_cod_sum=0, ptype=None):
        ...
        if month_data:
            if  not month_data.get('mvchc_sum') :
                mvchc_sum = 0
            else: 
                mvchc_sum = month_data.get('mvchc_sum')
            if  month_data.get('mto_pay_sum') :
                mto_pay_sum = month_data.get('mto_pay_sum')
            else: 
                mto_pay_sum = 0
            if  month_data.get('mrto_sum') :
                mrto_sum = month_data.get('mrto_sum')
            else: 
                mrto_sum = 0
            if  month_data.get('msdl_sum') :
                msdl_sum = month_data.get('msdl_sum')
            else: 
                msdl_sum = 0
            if  month_data.get('mreverse_sum') :
                mreverse_sum = month_data.get('mreverse_sum')
            else: 
                mreverse_sum = 0
            if  month_data.get('msdd_sum') :
                msdd_sum = month_data.get('msdd_sum')
            else: 
                msdd_sum = 0

            if  month_data.get('mfreight_sum') :
                mfreight_sum = month_data.get('mfreight_sum')
            else: 
                mfreight_sum = 0
            if  month_data.get('mfuel_sum') :
                mfuel_sum = month_data.get('mfuel_sum')
            else: 
                mfuel_sum = 0


            others = mvchc_sum + mto_pay_sum + mrto_sum + msdl_sum + mreverse_sum + msdd_sum 
            ...
            total = others + mfreight_sum + mfuel_sum + month_cod_sum
...
def write_pdsr_to_excel(date_str):
    file_name = "/non_ebs_pdsr_report_%s.xlsx"%(date_str)
    ...
    for data in data_gen:
        ...
        for dtz in data[1][5:]:
            if dtz:
                data_tobe_zipped.append(dtz)
            else:
                data_tobe_zipped.append(0)
        totals_row[1] = map(sum, zip(totals_row[1], data_tobe_zipped))
        ...
        for dtz in data[2][5:]:
            if dtz:
                ...
                data_tobe_zipped.append(0)
        totals_row[2] = map(sum, zip(totals_row[2], data_tobe_zipped))
