...
def generate_runcode_report():
    ...
    report = ReportGenerator('run_code_report_{0}.xlsx'.format(now.strftime('%Y-%m-%d')))
    year = now.year
    month = now.month
    day = int(now.day)-1
    col_heads = ('Run code',
...
#        bags=Connection.objects.using('local_ecomm').filter(runcode=runcode[0]).values_list('bags__bag_number', flat=True)
      ...
      #  if bag_count > 0:
        if shipment_count['ship_data__count']:
         # dest = runcode.destination.values_list('center_name', flat=True)
         # destinations = ', '.join(dest)
     #     row=(runcode.id,runcode.origin,runcode[2],len(bags),shipment_count['ship_data__count'],runcode.added_on)
          row=(runcode[0],runcode[3],runcode[4],len(bags),shipment_count['ship_data__count'],runcode[5])
          print row
