...
def get_pdsr_dispaly_data(date_str):
    ...
    day_data = Shipment.objects.using('local_ecomm').filter(shipext__product__in=[ebsppd, ebscod],
            shipment_date=date_str).values("shipper__code").\
            annotate(
                ship_count=Count('id'),
    ...
    ddata = Shipment.objects.using('local_ecomm').filter(shipext__product__in=[ebsppd, ebscod],
            shipment_date=date_str).values("shipper__code", "shipper__name", "shipext__product__product_name").\
            annotate(
                ship_count=Count('id'),
                total_cw=Sum('chargeable_weight'),
                freight_sum=Sum('order_price__freight_charge'),
    ...
    day_cod = Shipment.objects.using('local_ecomm').filter(shipext__product__in=[ebsppd, ebscod],
            shipment_date=date_str).exclude(rts_status = 1).values("shipper__code").\
        annotate(cod_sum=Sum('codcharge__cod_charge'),)

    day_cod_negative = Shipment.objects.using('local_ecomm').filter(shipment_date=date_str,
    ...
    month_data = Shipment.objects.using('local_ecomm').filter(shipext__product__in=[ebsppd, ebscod],
            shipment_date__year=year, shipment_date__month=month, shipment_date__lte=date_str)\
    ...
    mdata = Shipment.objects.using('local_ecomm').filter(shipext__product__in=[ebsppd, ebscod],
            shipment_date__year=year, shipment_date__month=month, shipment_date__lte=date_str)\
            .values("shipper__code", "shipper__name", "shipext__product__product_name").\
            annotate(
                mship_count=Count('id'),
                mtotal_cw=Sum('chargeable_weight'),
                mfreight_sum=Sum('order_price__freight_charge'),
    ...
    month_cod_cod = Shipment.objects.using('local_ecomm').filter(shipext__product__in=[ebsppd, ebscod],
            shipment_date__year=year, shipment_date__month=month, shipment_date__lte=date_str)\
    ...
    for d in day_product_wise:
        ptype = d.get('shipext__product__product_name')
        if ptype == 'ebscod':
            day_product_wise_dict[d['shipper__code']]['ebscod'] = d
        else:
            day_product_wise_dict[d['shipper__code']]['ebsppd'] = d
    ...
    for d in month_product_wise:
        ptype = d.get('shipext__product__product_name')
        if ptype == 'ebscod':
            month_product_wise_dict[d['shipper__code']]['ebscod'] = d
        else:
            month_product_wise_dict[d['shipper__code']]['ebsppd'] = d
    ...
    def get_prodwise_row(day_data, month_data, day_cod_sum=0, month_cod_sum=0, ptype=None):
        ...
        if month_data:
            #print 'month data exists..', month_data
    ...
    for cid in cids:
        ...
        if code:
            day_coddata = day_product_wise_dict[code].get('ebscod')
            month_coddata = month_product_wise_dict[code].get('ebscod')

            day_ppddata = day_product_wise_dict[code].get('ebsppd')
            month_ppddata = month_product_wise_dict[code].get('ebsppd')
            ...
            cod_row = get_prodwise_row(day_coddata, month_coddata, day_cod_sum=day_cod_sum, month_cod_sum=month_cod_sum, ptype='EBS COD')
            #print cod_row

            # get ppd row
            #print 'prepare ppd row.............'
            ppd_row = get_prodwise_row(day_ppddata, month_ppddata, ptype='EBS PPD')
            #print ppd_row
...
def write_pdsr_to_excel(date_str):
    file_name = "/ebs_pdsr_report_%s.xlsx"%(date_str)
    ...
    #ppd - day
    total_00 = totals_row[0][0] if totals_row[0][0] else 1
    total_01 = totals_row[0][1] if totals_row[0][1] else 1
    totals_row[0][7] = totals_row[0][1]/total_00
    totals_row[0][8] = totals_row[0][6]/total_00
    totals_row[0][9] = totals_row[0][6]/total_01

    #ppd - month
    total_010 = totals_row[0][10] if totals_row[0][10] else 1
    total_011 = totals_row[0][11] if totals_row[0][11] else 1
    totals_row[0][17] = totals_row[0][11]/total_010
    totals_row[0][18] = totals_row[0][16]/total_010
    totals_row[0][19] = totals_row[0][16]/total_011

    #cod - day
    total_10 = totals_row[1][0] if totals_row[1][0] else 1
    total_11 = totals_row[1][1] if totals_row[1][1] else 1
    totals_row[1][7] = totals_row[1][1]/total_10
    totals_row[1][8] = totals_row[1][6]/total_10
    totals_row[1][9] = totals_row[1][6]/total_11

    #cod - month
    total_110 = totals_row[1][10] if totals_row[1][10] else 1
    total_111 = totals_row[1][11] if totals_row[1][11] else 1
    totals_row[1][17] = totals_row[1][11]/total_110
    totals_row[1][18] = totals_row[1][16]/total_110
    totals_row[1][19] = totals_row[1][16]/total_111

    # total - day
    total_20 = totals_row[2][0] if totals_row[2][0] else 1
    total_21 = totals_row[2][1] if totals_row[2][1] else 1
    totals_row[2][7] = totals_row[2][1]/total_20
    totals_row[2][8] = totals_row[2][6]/total_20
    totals_row[2][9] = totals_row[2][6]/total_21

    # total - month
    total_210 = totals_row[2][10] if totals_row[2][10] else 1
    total_211 = totals_row[2][11] if totals_row[2][11] else 1
    totals_row[2][17] = totals_row[2][11]/total_210
    totals_row[2][18] = totals_row[2][16]/total_210
    totals_row[2][19] = totals_row[2][16]/total_211

    # EBS COD row
    sheet.write(row_num, 2, 'EBS PPD TOTAL')
    ...
    # EBS COD row
    sheet.write(row_num+1, 2, 'EBS COD TOTAL')
