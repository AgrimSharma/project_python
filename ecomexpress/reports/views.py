...
from reports.reports_download import get_report
...
def get_city_details(request):
    if request.method == 'GET':
        ...
        origin_sc = ServiceCenter.objects.filter(city__state_id=state)
        html = render_to_string("reports/state_details.html", {'cities': origin_sc})
    return {'html': html}
...
def get_region_details(request):
    if request.method == 'GET':
        ...
        sc = ServiceCenter.objects.filter(city__region_id__in=region)
        sc_state = ServiceCenter.objects.filter(city__region_id__in=region).values_list('city__state_id')
        state = State.objects.filter(id__in=sc_state)
        region_html = render_to_string("reports/region_details.html", {'states': state})
        html = render_to_string("reports/state_details.html", {'cities': sc})
    return {'html': html, 'region_html': region_html}
...
def report_search(request):
    if request.method == 'GET':
       report_type = request.GET['reports']
       customer = request.GET['customers']
       origin_city =request.GET.getlist('origin_city[]')
       dest_city = request.GET.getlist('dest_city[]')
       origin_state = request.GET['origin_state']
       dest_state = request.GET['dest_state']
       origin_region = request.GET['origin_region']
       dest_region = request.GET['dest_region']
       ...
       for city in dest_city:
           c = city.strip('"')
           dest.append(int(c)) 
       filename = get_report(report_type=report_type, customer=customer, origin_city=origin, dest_city=dest,
           origin_state=origin_state, dest_state=dest_state, origin_region=origin_region, dest_region=dest_region, from_date=from_date,
           to_date=to_date)
       file_name = '/static/uploads/reports/{0}'.format(filename)
...
def report_search(request):
    if request.method == 'POST':
       ...
       origin_city =request.POST.getlist('origin_city[]')
       dest_city = request.POST.getlist('dest_city[]')
       origin_state = request.POST['origin_state']
       dest_state = request.POST['dest_state']
       #return HttpResponse(dest_state)
       ...
       for city in dest_city:
           c = city.strip('"')
           dest.append(int(c))
       filename = get_report(customer=customer, origin_city=origin, dest_city=dest, report_type=report_type,
           origin_state=origin_state, dest_state=dest_state, origin_region=origin_region, dest_region=dest_region, from_date=from_date,
           to_date=to_date)
       #return HttpResponse(filename)
       #file_name = '/static/uploads/reports/{0}'.format(filename)
       #return {'file_name': file_name}
       return HttpResponseRedirect('/static/uploads/reports/'+filename)  
...
def ndr(request):
    ...
    if request.POST:
        ...
        origin = []
        for city in origin_city:
            origin.append(int(city))
        dest = []
        for city in dest_city:
            dest.append(int(city))
        #return HttpResponse("%s" % dest)
        ...
        q = Q()
        if origin_city:
            #city = ServiceCenter.objects.filter(id__in=origin)
            q = q & Q(pickup__service_centre_id__in=origin)
        if dest_city:
            #city = ServiceCenter.objects.filter(id__in=dest)
            q = q & Q(current_sc_id__in=dest)
        if origin_state:
            if origin_state != "0":
               #state_id=State.objects.filter(id=origin_state)
               #q = q & Q(pickup__service_centre__city__state__in=state_id)
                q = q & Q(pickup__service_centre__city__state_id=origin_state)
        if dest_state:
            if dest_state != "0":
               #state_id=State.objects.filter(id=dest_state)
               #q = q & Q(current_sc__city__state__in=state_id)
                q = q & Q(current_sc__city__state_id=dest_state)
        if origin_region:
            if origin_region != "0":
               #region_id=Region.objects.filter(id__in=origin_region)
               #q = q & Q(pickup__service_centre__city__region_id__in=region_id)
                q = q & Q(pickup__service_centre__city__region_id=origin_region)
        if dest_region:
            if dest_region != "0":
               #region_id=Region.objects.filter(id__in=dest_region)
               #q = q & Q(current_sc__city__region_id__in=region_id)
                q = q & Q(current_sc__city__region_id=dest_region)
        ...
        q = q & Q(status__in=[7,8])
        q = q & Q(rts_status = 0)
        rtsq = Q(reason_code__isnull = True )| ~Q(reason_code_id__in = [55,46,44,37,32,29,53,52,6,5,4,1]) 
        q = q & rtsq
        ...
        shipments = Shipment.objects.using("local_ecomm").filter(q)
        #return HttpResponse("%s" % shipments[0].current_sc)
        shipment_info={}
        download_list = []
    ...
    for a in shipments:
        if a.reason_code_id != 1:
           # if a.reason_code_id != 1 and a.reason_code_id !=34 and a.reason_code_id !=5 and a.ref_airwaybill_number !="" and a.return_shipment !=2:
                teleList = TeleCallingReport.objects.using('local_ecomm').filter(shipment_id=a.id)
                tele = ''
                if teleList:
                ...
                if tele:
                    ...
                    tele_date = tele.added_on
                first_inscan_hub_date = ''
                last_inscan_hub_date = ''
                inscan_dc_date = ''
                inscan_dc = ''
                first_attempted_date = ''

                inscan_hub_date = ''
                first_inscan_hub_locn = ''
                last_inscan_hub_locn = ''
                rts_updated_on = ""
                ...
                if not shipment_info.get(a):
                    ...
                    monthdir = upd_time.strftime("%Y_%m")
                    shipment_history = get_model('service_centre', 'ShipmentHistory_%s'%(monthdir))
                    #history = shipment_history.objects.using('local_ecomm').filter(shipment=a).exclude(reason_code__code__in=[216,208,202,311,200,111, 777, 999, 888, 333, 310,213])
                    ...
                    upd_date = a.updated_on
                   #if history.filter(status=50):
                   #    inscan_hub_date = history.filter(status=50)
                   #    first_inscan_hub_date = inscan_hub_date[0].updated_on.date()
                   #    first_inscan_hub_locn = inscan_hub_date[0].current_sc.center_shortcode
                   #    last_inscan_hub_date = history.filter(status=50).latest('updated_on').updated_on.date()
                   #    last_inscan_hub_locn = history.filter(status=50).latest('updated_on').current_sc.center_shortcode
                   #if history.filter(status=6):
                   #    inscan_dc_date = history.filter(status=6).latest('updated_on').updated_on.date()
                   #    inscan_dc = history.filter(status=6).latest('updated_on').current_sc.center_shortcode
                    supd = a.statusupdate_set.all()
                    if supd:
                        first_attempted_date = supd[0].added_on.date()
                   ...
                   #    remarks = su.remarks
                    ...
                    rsd = ''
                    if history:
                       history = history.latest('updated_on')
                       if history.reason_code:
                           rs = history.reason_code.code
                           rsd = history.reason_code.code_description
                    ...
                    if a.ref_airwaybill_number and Shipment.objects.using('local_ecomm').filter(airwaybill_number = a.ref_airwaybill_number):
                       ...
                       # rts_updated_on = rts_history
                                    if rts_history:
                                        ...
                                        rts_updated_on = ""
                                    u = (a.airwaybill_number, a.order_number, a.added_on.date(), a.pickup.service_centre.center_name, sc, a.pickup.service_centre.city.region.region_name, a.original_dest.city.region.region_name, a.pickup.service_centre.city.state.state_name, a.original_dest.city.state.state_name, a.shipper, a.consignee, a.collectable_value, upd_date, a.mobile, rs, rsd, remarks,a.deliveryoutscan_set.filter().only('id').count(),comment,a.current_sc, get_internal_shipment_status(a.status),first_inscan_hub_date,first_inscan_hub_locn,last_inscan_hub_date, last_inscan_hub_locn, inscan_dc_date,inscan_dc, tele_date, first_attempted_date)
                    else:
                        ...
                        u = (a.airwaybill_number, a.order_number, a.added_on.date(), a.pickup.service_centre.center_name, sc, a.pickup.service_centre.city.region.region_name, a.original_dest.city.region.region_name, a.pickup.service_centre.city.state.state_name, a.original_dest.city.state.state_name,a.shipper, a.consignee, a.collectable_value, upd_date, a.mobile, rs, rsd, remarks,a.deliveryoutscan_set.filter().only('id').count(),comment,a.current_sc, get_internal_shipment_status(a.status),first_inscan_hub_date, first_inscan_hub_locn, last_inscan_hub_date, last_inscan_hub_locn, inscan_dc_date,inscan_dc, tele_date, first_attempted_date)
        if report_type == "/reports/ndr/":
            report = ReportGenerator('NDR_report_{0}_{1}%s.xlsx'.format(date_from, date_to) % datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
            col_heads = ('Airwaybill no', 'Order no', 'Pickup Date', 'Origin SC', 'Destination SC', 'Origin Region', 'Destn Region', 
                         'Origin State', 'Destn State','Shipper', 'Consignee', 'Collectable Value', 'Status Updated On', 'Mobile no', 
                         'Reason Code','Reason Code Description','Remarks', 'No. of attempts', 'Comment', 'Current SC', 'Status',
                         'First Inscan Hub Date','First Inscan Hub Locn', 'Last Inscan Hub Date', 'Last Inscan Hub Locn',
                         'Inscan at DC Date','Inscanned DC','Telecalling Uploaded On', 'First Attempted Date')
...
def not_outscan(request):
	...
	if sc == "0":
        ...
        shipments = Shipment.objects.using('local_ecomm').filter(
            q, status=6).exclude(status__range=(7,9)).exclude(
            status_type=5).exclude(rts_status=2).exclude(
            reason_code__code__in=[333, 310, 302, 208, 999, 777]).exclude(
            shipper_id=12)
        shipment_info={}
        download_list = []
...
def bag_detail(request):
    if request.POST:
        ...
        for bag in bags:
            ...
            if bag_detail:
                for a in bag_detail:
                    shipment_info[a] = get_internal_shipment_status(a.status)
                    dt = a.updated_on
                    if not dt:
                       dt = ""
                    else:
...
def data_uploaded_status(request,stat):
	...
	if sc == "0":
        ...
        shipments = Shipment.objects.using('local_ecomm').filter(
            q, status__in=stat).exclude(
            reason_code__code__in=[333, 310, 302, 208, 999, 777]
        ).exclude(rts_status=1).exclude(rto_status=1)
...
def performance_analysis_location(request, type=0, rtype=0):
    ...
    if type == 1:
       ...
       #q = q & Q(added_on__range=(start_date,end_date))
    elif request.POST:
        report_type = request.POST['report_type']
        date_from = request.POST['date_from']
        date_to = request.POST['date_to']
...
def pickup_dashbaord(request):
    ...
    return render_to_response("integration_services/modal_base.html",
                               context_instance=RequestContext(request))
