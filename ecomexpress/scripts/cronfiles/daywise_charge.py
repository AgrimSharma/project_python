...
def get_daywise_charge_report():
    ...
    #year = 2014
    #month = 04
    freight_data = Shipment.objects.filter(shipper__activation_status=True,
        shipment_date__month=month,
        shipment_date__year=year).values("shipment_date").\
        annotate(Count('id'),
    ...
    cod_charges = Shipment.objects.filter(shipper__activation_status=True,
        shipment_date__month=month,
        shipment_date__year=year).exclude(rts_status = 1).exclude(shipment_date__gt=now).values("shipment_date").\
        annotate(cod_charge=Sum('codcharge__cod_charge'), collectable_value=Sum('collectable_value'))

    cod_charges_negative = Shipment.objects.filter(shipper__activation_status=True,
        shipment_date__month=month,
    ...
    fd = Shipment.objects.filter(shipper__activation_status=True,
        shipment_date__month=month,
        shipment_date__year=year).exclude(shipment_date__gt=now).\
        aggregate(Count('id'),
    ...
    cod_charges = Shipment.objects.filter(shipper__activation_status=True,
        shipment_date__month=month,
        shipment_date__year=year).exclude(shipment_date__gt=now).exclude(rts_status = 1).\
