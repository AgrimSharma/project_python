...
def volumetric_weight():
    shipments=Shipment.objects.filter( shipment_date__range=(report_date,nextmonth_date)).\
           values('pickup__service_centre__city__city_shortcode','airwaybill_number',
                 ...
                 'shipment_date','current_sc__center_name','item_description').exclude(chargeable_weight = F("actual_weight"))
#    sh=[]
#for a in shipments.iterator():
#    u = (a.airwaybill_number,a.actual_weight, a.volumetric_weight, a.length, a.breadth, a.height, a.chargeable_weight, a.pickup.service_centre, a.original_dest, a.shipper.name, a.shipper.code, ,a.inscan_date,a.current_sc)
#    sh.append(u)

    file_name = "/volumetric_weight_report_%s.xlsx"%(datetime.datetime.now().strftime("%d%m%Y%H%M%S%s"))
    ...
    sheet.write(0, 4, "Volumetric Weight Report", header_format)
    row_num = 3
...
gq = volumetric_weight()
gq_split = gq.split('/ecomexpress')
gq_path = 'http://billing.ecomexpress.in'+gq_split[1]

subject = "Volumetric Weight Shipments Report"
message = "Please find the link below of shipments weighing more than 20KG \n %s"%(gq_path)
from_email = "support@ecomexpress.in"
to_email = ("sravank@ecomexpress.in", "jaideeps@ecomexpress.in", "nareshb@ecomexpress.in", "jignesh@prtouch.com", "onkar@prtouch.com","samar@prtouch.com","samar@prtouch.com")
