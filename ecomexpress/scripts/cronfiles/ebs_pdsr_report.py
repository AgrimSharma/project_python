...
from reports.ebs_pdsr_report import write_pdsr_to_excel
import settings
...
def main():
	...
	today_str = before.strftime('%Y-%m-%d')
        ...
        #add more
        total_rts = Shipment.objects.filter(shipment_date=today_str,rts_status=1, shipext__product__in=[ebscod, ebsppd]).only('id').count()
        cod_rts_count = Shipment.objects.filter(shipment_date=today_str,rts_status=1,shipext__product=ebscod).only('id').count()
        ppd_rts_count = Shipment.objects.filter(shipment_date=today_str,rts_status=1,shipext__product=ebsppd).only('id').count()

	fre_total = Shipment.objects.filter(shipment_date=today_str, shipext__product__in=[ebscod, ebsppd]).only('id').count()
        cod_total = Shipment.objects.filter(shipment_date=today_str,shipext__product=ebscod).only('id').count()
        ppd_total = Shipment.objects.filter(shipment_date=today_str,shipext__product=ebsppd).only('id').count()
        cod_fresh = cod_total-cod_rts_count
        ppd_fresh = ppd_total-ppd_rts_count
        total_fresh = fre_total - total_rts
        ebs_ppd = Shipment.objects.filter(shipment_date=today_str,shipext__product=ebsppd).only('id').count()
        ebs_cod =Shipment.objects.filter(shipment_date=today_str,shipext__product=ebscod).only('id').count()
        rvb_totlal = 0
        rvb_cod = 0
        rvb_ppd = 0
	...
	m_total_rts = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,rts_status=1, shipext__product__in=[ebscod, ebsppd]).only('id').count()
	m_cod_rts_count = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,rts_status=1,shipext__product=ebscod).only('id').count()
	m_ppd_rts_count = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,rts_status=1,shipext__product=ebsppd).only('id').count()
	m_fre_total = Shipment.objects.filter(shipment_date__lte=today_str,shipment_date__month=now.month, shipext__product__in=[ebscod, ebsppd]).only('id').count()
	m_cod_total = Shipment.objects.filter(shipment_date__lte=today_str,shipment_date__month=now.month,shipext__product=ebscod).only('id').count()
	m_ppd_total = Shipment.objects.filter(shipment_date__lte=today_str,shipment_date__month=now.month,shipext__product=ebsppd).only('id').count()
	m_cod_fresh = m_cod_total-m_cod_rts_count
	m_ppd_fresh = m_ppd_total-m_ppd_rts_count
	m_total_fresh = m_fre_total - m_total_rts
	m_ebs_ppd = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,shipext__product=ebsppd).only('id').count()
	m_ebs_cod =Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,shipext__product=ebscod).only('id').count()
	m_rvb_totlal = 0
	m_rvb_cod = 0
	m_rvb_ppd = 0
	...
	s1 = smtplib.SMTP('i.prtouch.com', 26)
        ...
        content = '<html><body><p>Dear Team,</p><p>Please find  attached Previous Day Sales Report for EBS Shipments.</p><p>Summary of report follows</p><table  border="1" cellpadding="10"  cellspacing="0"><tr><th></th><th>EBS PPD</th><th>EBS COD</th><th>Total</th></tr>'
        count = 0      
	...
	content = content+'</table><h2>Daily</h2><table  border="1" cellpadding="10"  cellspacing="0"><tr><th></th><th>EBS PPD</th><th>EBS COD</th><th>Total</th></tr>'
	...
	content = content+'</table><h2>Monthly</h2><table  border="1" cellpadding="10"  cellspacing="0"><tr><th></th><th>EBS PPD</th><th>EBS COD</th><th>Total</th></tr>'
	content = content+'<tr><td bgcolor="Yellow">Total Rts</td><td> '+str(m_ppd_rts_count)+' </td><td> '+str(m_cod_rts_count)+'</td><td>'+str(m_total_rts)+'</td></tr>'
        ...
        #msg = EmailMultiAlternatives('Daily Sales Report', text, msg['from'], to_mail_ids) 
        msg['Subject'] = 'Daily Sales Report(EBS)'
        msg.attach(html)    
        msg.attach(attach)
