...
def main():
	...
	m_total_rts = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,rts_status=1).only('id').count()
	m_cod_rts_count = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,rts_status=1,product_type='cod').only('id').count()
	m_ppd_rts_count = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,rts_status=1,product_type='ppd').only('id').count()
	m_fre_total = Shipment.objects.filter(shipment_date__lte=today_str,shipment_date__month=now.month).only('id').count()
	m_cod_total = Shipment.objects.filter(shipment_date__lte=today_str,shipment_date__month=now.month,product_type='cod').only('id').count()
	m_ppd_total = Shipment.objects.filter(shipment_date__lte=today_str,shipment_date__month=now.month,product_type='ppd').only('id').count()
	m_cod_fresh = m_cod_total-m_cod_rts_count
	m_ppd_fresh = m_ppd_total-m_ppd_rts_count
	m_total_fresh = m_fre_total - m_total_rts
	m_ebs_ppd = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,airwaybill_number__istartswith=3).only('id').count()
	m_ebs_cod =Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,airwaybill_number__istartswith=4).only('id').count()
	m_rvb_totlal = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,reverse_pickup=1).only('id').count()
	m_rvb_cod = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,reverse_pickup=1,product_type='cod').only('id').count()
	m_rvb_ppd = Shipment.objects.filter(shipment_date__month=now.month,shipment_date__lte=today_str,reverse_pickup=1,product_type='ppd').only('id').count()
	...
	s1 = smtplib.SMTP('i.prtouch.com', 26)
        ...
        #smtp_to_mail_ids = ["jignesh@prtouch.com", "jinesh@prtouch.com", "jaideeps@ecomexpress.in", "satyak@ecomexpress.in"]
        #smtp_to_mail_ids = ["jignesh@prtouch.com" ]
	#to_mail_ids =("arun@prtouch.com","onkar@prtouch.com","onkar.tonge@gmail.com")
	to_mail_ids =("jignesh@prtouch.com","arun@prtouch.com","onkar@prtouch.com","onkar.tonge@gmail.com")
	...
	content = content+'<tr><td bgcolor="Yellow">Total Rts</td><td> '+str(m_ppd_rts_count)+' </td><td> '+str(m_cod_rts_count)+'</td><td>'+str(m_total_rts)+'</td></tr>'
        ...
        text =("Dear Team, \nPlease find  attached Previous Day Sales Report, without ebs shipments. \n\nRegards\n\nSupport Team\n total=%s"%(content))
        html =content
        html =MIMEText(html, 'html')
        #msg = EmailMultiAlternatives('Daily Sales Report', text, msg['from'], to_mail_ids) 
        msg['Subject'] = 'Daily Sales Report (Excluding EBS)'
        msg.attach(html)    
        msg.attach(attach)
