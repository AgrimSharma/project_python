...
from ecomexpress.utils import get_vendor
...
@login_not_required
@csrf_exempt
def api_form3(request):                                                                                                                          
    return render_to_response('api/api_form3_without_awb.html')                   
...
@login_not_required
@csrf_exempt
def awb_detail_check(xml_dict, capi):
     error_list = {}
     dup_awb = []
     reason_comment = ""
     for record in xml_dict:
         ...
         collectable_value = record["COLLECTABLE_VALUE"] 
         ...
         if not airwaybill_num:
           airwaybill_num = order_number
         if not error_list.get(airwaybill_num):
           error_list[airwaybill_num] = {}

         if not pickup_pincode:
                error_list[airwaybill_num]["pickup_pincode"] = "No Pickup Pincode Provided"
                reason_comment = reason_comment + " NO_PICKUP_PINCODE"
                #continue
         else:
                if not Pincode.objects.filter(pincode=pickup_pincode, status=1):
                    error_list[airwaybill_num]["pickup_pincode"] = "Invalid Pickup Pincode Provided"
                    reason_comment = reason_comment + " INVALID_PICKUP_PINCODE"
                    #continue

         if not pincode:
                error_list[airwaybill_num]["pincode"] = "No Consignee Pincode Provided"
                reason_comment = reason_comment + " NO_CONSIGNEE_PINCODE"
                #continue
         else:
                if not Pincode.objects.filter(pincode=pincode, status=1):
                    error_list[airwaybill_num]["pincode"] = "Invalid Consignee Pincode Provided"
                    reason_comment = reason_comment + " INVALID_CONSIGNEE_PINCODE"
                    #continue


         if product_type not in ["cod","ppd"]:
                error_list[airwaybill_num]["product"] = "Invalid Product Type"
                reason_comment = reason_comment + " INVALID_PRODUCT_TYPE"
                #continue

         if product_type == "cod":
                if not collectable_value  or collectable_value == 0.0: 
                    error_list[airwaybill_num]["collectable_value"] = "Invalid collectable_value"
                    reason_comment = reason_comment + " INVALID_COLLECTABLE_VALUE"
                    #continue
                try:
                    float(collectable_value)
                except:
                    error_list[airwaybill_num]["collectable_value"] = "Invalid collectable_value"
                    reason_comment = reason_comment + " INVALID_COLLECTABLE_VALUE"
                    #continue
         ...
         if not actual_weight: 
                error_list[airwaybill_num]["weight_dimension"] = "Incorrect Weight and dimensions"
                reason_comment = reason_comment + " NO_ACTUAL_WEIGHT"
                #continue

        #if not actual_weight and not volumetric_weight and not length and not breadth and not height: 
        #       error_list[airwaybill_num]["weight_dimension"] = "Incorrect Weight and dimensions"
        #       continue
         ...
         if actual_weight: 
                try:
                    ...
                    error_list[airwaybill_num]["weight_dimension"] = "Incorrect Weight and dimensions"
                    reason_comment = reason_comment + " INCORRECT_ACTUAL_WEIGHT"
                    #continue
         ...
         if volumetric_weight: 
                try:
                    ...
                    error_list[airwaybill_num]["weight_dimension"] = "Incorrect Weight and dimensions"
                    reason_comment = reason_comment + " INCORRECT_VOLUMETRIC_WEIGHT"
                    #continue
         ...
         if length: 
                try:
                    ...
                    error_list[airwaybill_num]["weight_dimension"] = "Incorrect Weight and dimensions"
                    reason_comment = reason_comment + " INCORRECT_LENGTH"
                    #continue
         ...
         if breadth: 
                try:
                    ...
                    error_list[airwaybill_num]["weight_dimension"] = "Incorrect Weight and dimensions"
                    reason_comment = reason_comment + " INCORRECT_BREADTH"
                    #continue
         ...
         if height: 
                try:
                    ...
                    error_list[airwaybill_num]["weight_dimension"] = "Incorrect Weight and dimensions"
                    reason_comment = reason_comment + " INCORRECT_HEIGHT"
                    #continue
         ...
         if airwaybill_num not in dup_awb:
                ...
                reason_comment = reason_comment + " DUPLICATE_AWB"
         ...
         if Shipment.objects.filter(airwaybill_number=airwaybill_num):
               ...
               if (awb_num.status >=2 or awb_num.return_shipment > 0):
                    error_list[airwaybill_num]["awb_used"] = "Airwaybill already in use"
                    reason_comment = reason_comment + " AWB_IN_USE"
                    #continue           

         if error_list[airwaybill_num]:
               error_list[airwaybill_num]["reason_comment"] = reason_comment
               continue
...
def awb_check(xml_dict, capi):
     ...
     for record in xml_dict:
        ...
        #if not pincode:
        #       error_list[airwaybill_num]["pincode"] = "No Pickup Pincode Provided"
        #       continue
        #else:
        #       if not Pincode.objects.filter(pincode=pincode, status=1):
        #           error_list[airwaybill_num]["pincode"] = "Invalid Pickup Pincode Provided"
        #           continue
...
def subcust_check(xml_dict,capi):
    ...
    for record in xml_dict:
        ...
        if Shipper.objects.filter(alias_code=sub_customer_id , customer=capi.customer):
           sub_customer = Shipper.objects.get(alias_code=sub_customer_id, customer = capi.customer)
           if not sub_customer.id in subcustomers_list:
                  subcustomers_list.append(sub_customer.id)
        else:
                if not error_list.get(order_num):
                      error_list[order_num] = {}
                      error_list[order_num]["vendor"] = \
                         "Subcustomer {0} not found! Please check again.".format(sub_customer_id)
    if not error_list:
...
@login_not_required
@csrf_exempt
def pickup_create(subcust_list, reverse_pickup = 0):
    pickup_dict = {}
    subcustomer_list = {}
    now = datetime.datetime.now()
    for subc in subcust_list:
        ...
        pickup = PickupRegistration.objects.filter(customer_code = subcust.customer,
                 subcustomer_code=subcust, status = 1).filter(Q(pickup_time__gte="07:00:00", 
        ...
        if pickup:
            ...
            pincode_sc_map = PickupPincodeServiceCentreMAP.objects.filter(pincode = int(subcust.address.pincode))
            
            if pincode_sc_map :
                sc = pincode_sc_map[0].service_center
            else:
                pincode = Pincode.objects.get(pincode = int(subcust.address.pincode))
        ...
        pickup_dict[subcust.alias_code] = pickup
...
def pincode_pickup_create(record, capi, reverse_pickup):
    ...
    subcustomer = Shipper.objects.get(id=sub_customer_id)
    pickup = PickupRegistration.objects.filter(customer_code = capi.customer,
             subcustomer_code_id=sub_customer_id, pincode = pincode, status=0).filter(Q(pickup_time__gte="07:00:00", 
    ...
    if pickup:
        ...
        pickup = PickupRegistration.objects.create(customer_code=capi.customer,subcustomer_code=subcustomer,
...
@login_not_required
@csrf_exempt
def pincode_revpickup_create(record, capi, reverse_pickup):
    pickup_dict = {}
    subcustomer_list = {}
    now = datetime.datetime.now()
    error = False
    try:
        pincode = Pincode.objects.get(pincode = int(record["DROP_PINCODE"]))
    except:
        error = True
    pickup = PickupRegistration.objects.filter(customer_code = capi.customer,
             subcustomer_code=None, pincode = pincode.pincode).filter(Q(pickup_time__gte="07:00:00", 
             pickup_date=from_time) & Q(pickup_time__lte="07:00:00", pickup_date=to_time)).filter(reverse_pickup=reverse_pickup).\
             order_by("-pickup_date", "-pickup_time")
    ...
    if pickup:
        ...
        pincode_sc_map = PickupPincodeServiceCentreMAP.objects.filter(pincode = int(record["DROP_PINCODE"]))
        ...
        pickup = PickupRegistration.objects.create(customer_code=capi.customer,subcustomer_code=None,
                 pickup_time=now,pickup_date=now,mode_id=1,customer_name=capi.customer.name,
                 address_line1=record["DROP_ADDRESS_LINE1"],address_line2=record["DROP_ADDRESS_LINE2"],
                 pincode=record["DROP_PINCODE"],
                 mobile=record["DROP_MOBILE"],telephone=record["DROP_PHONE"],pieces=4,
                 actual_weight=1.2,volume_weight=2.1,service_centre=sc, reverse_pickup=reverse_pickup)
    pickup_dict[record["DROP_PINCODE"]] = pickup
...
#        password = (file_contents['SOAP-ENV:Envelope']['password'])
    ...
    if request.POST:
            ...
            #return HttpResponse( shipment_list)
            awb_check_list = awb_check(shipment_list, capi) 

            if awb_check_list <> 1:
               return HttpResponse(str(awb_check_list))
            ...
            subcustomer_check = subcust_check(shipment_list, capi) 
    ...
    #                mywriter.writerow([str(record)])
        ...
        #            airwaybill_num = record["AWB_NUMBER"]
         ...
         #           awb_num = AirwaybillNumbers.objects.filter(airwaybill_number=airwaybill_num).update(status=1)
                    ...
                    #return HttpResponse( org_pincode)
...
def fetch_airwaybill(request):
    ...
    order_numbers = request.POST['order_number']
    ship_type = request.POST['ship_type']

    order_numbers = order_numbers.split(",")
    xml_output = '<?xml version="1.0" encoding="UTF-8" standalone="no"?>\n<AIRWAYBILL-OBJECTS>\n'
    awbc = AirwaybillCustomer.objects.create(customer=capi.customer, type=ship_type, quantity=len(order_numbers))
    for order_number in order_numbers:
        ...
        xml_output = xml_output + "<AIRWAYBILL> <order_id>%s</order_id> <airwaybill_number>%d</airwaybill_number> </AIRWAYBILL>\n" % (order_number, airwaybill_number)
...
def create_shipper(request):
    ...
    if not error_dict:
        ...
        for k, v in error_dict.items():
            errors += k + ' : ' + ','.join(v) + '\n'
        success_str = "<string><root><message>Vendors Added Successfully</message></root><error>{0}</error></string>".format(errors)
...
@login_not_required
@csrf_exempt
def api_create_awb_shipment(request):
    """
    ...
    #           return HttpResponse("Unauthorised Request")
	    ...
	    rev_pickup_list = []
            ...
            for rev_awb_check_value in shipment_list:
                if rev_awb_check_value["PRODUCT"].lower() not in  ["ppd","cod"]:
                    error_xml_output = '<?xml version="1.0" encoding="UTF-8" standalone="no"?><RESPONSE-OBJECTS><RESPONSE-TYPE>ERROR</RESPONSE-TYPE>\n'
                    error_xml_output = error_xml_output + "<RESPONSE-COMMENT>Incorrect Product Type! only 'cod' or 'ppd' allowed!</RESPONSE-COMMENT></RESPONSE-OBJECTS>"
                    return HttpResponse(
                        error_xml_output, 
                        content_type="application/xhtml+xml"
                    )    
            ...
            awb_check_list = awb_check(awb_check_shiplist, capi)
            #if awb_check_list <> 1:
            #    return HttpResponse(str(awb_check_list))

            awb_detail_check_list = awb_detail_check(shipment_list, capi)
            #if awb_detail_check_list <> 1:
            #    return HttpResponse(str(awb_detail_check_list))
            #return HttpResponse("awb_detail_check_list : %s" % awb_detail_check_list)

            if awb_check_list <> 1 and awb_detail_check_list <> 1:
                awb_error_list = dict(awb_check_list.items() + awb_check_list.items())
            elif awb_check_list <> 1: 
                awb_error_list = awb_check_list
            elif awb_detail_check_list <> 1: 
                awb_error_list = awb_detail_check_list
                #return HttpResponse("%s " % (awb_error_list))

            if awb_check_list <> 1 or awb_detail_check_list <> 1:
                error_xml_output = '<?xml version="1.0" encoding="UTF-8" standalone="no"?><RESPONSE-OBJECTS><RESPONSE-TYPE>ERROR</RESPONSE-TYPE>\n<AIRWAYBILL-OBJECTS>\n'
                if awb_error_list <> 1: 
                ...
                error_xml_output = error_xml_output + "<RESPONSE-COMMENT>Reasons along with each awb</RESPONSE-COMMENT></RESPONSE-OBJECTS>"
            ...
            #return HttpResponse("%s Request" % ship_type_count)
            #order_numbers = request.POST['order_number']
           
            #ship_type = request.POST['ship_type']

            #order_numbers = order_numbers.split(",")
            xml_output = '<?xml version="1.0" encoding="UTF-8" standalone="no"?><RESPONSE-OBJECTS><RESPONSE-TYPE>SUCCESS</RESPONSE-TYPE>\n<AIRWAYBILL-OBJECTS>\n'
...
#           #return HttpResponse(file_contents['ECOMEXPRESS-OBJECTS']['SHIPMENT'])
            ...
            for record in shipment_list:
                    if not svalue["AWB_NUMBER"]:
                        ...
                        order_awb_numbers[record["ORDER_NUMBER"]] = record["AWB_NUMBER"]
                        xml_output = xml_output + "<AIRWAYBILL> <order_id>%s</order_id> <airwaybill_number>%s</airwaybill_number> </AIRWAYBILL>\n" % (record["ORDER_NUMBER"], record["AWB_NUMBER"])
                    if not int(record["AWB_NUMBER"]):
                        return HttpResponse("Error, No airway bill number found for order number %s. Please note that nothing is updated" % record["ORDER_NUMBER"] )
        ...
        #            airwaybill_num = record["AWB_NUMBER"]
            ...
            #            continue
                    if record["VENDOR_ID"]:
                        ...
                        if not record["PICKUP_PINCODE"]:
                            continue
                        #pickup_data, error = pickup_create(subcustomer_check)
                        #return HttpResponse("%s" %record)
                        pickup_data, error = pincode_pickup_create(record, capi, rev_pickup_flag)
                        #return HttpResponse(file_contents['ECOMEXPRESS-OBJECTS']['SHIPMENT'])
                        if error:
                            return HttpResponse(str(error))            
                        #return HttpResponse(file_contents['ECOMEXPRESS-OBJECTS']['SHIPMENT'])
                        pickup = pickup_data.get(record["PICKUP_PINCODE"])
                        if not pickup:
                            continue
         ...
         #           awb_num = AirwaybillNumbers.objects.filter(airwaybill_number=airwaybill_num).update(status=1)
            ...
            xml_output = xml_output + "</AIRWAYBILL-OBJECTS>"
            xml_output = xml_output + "<RESPONSE-COMMENT>Shipments Updated</RESPONSE-COMMENT></RESPONSE-OBJECTS>"
            return HttpResponse(
                xml_output, 
                content_type="application/xhtml+xml"
            )
...
@login_not_required
@csrf_exempt
def api_create_revawb_shipment(request):
    pid=1
    awb_overweight=[]
    subCustomers_list=[]
    capi = False
	    ...
	    rev_pickup_list = []
            ...
            for rev_awb_check_value in shipment_list:
                if rev_awb_check_value["PRODUCT"].lower() == "rev":
                    ...
                    return HttpResponse("Only Reverse Pickup Allowed")
    ...
    #                mywriter.writerow([str(record)])
        ...
        #            airwaybill_num = record["AWB_NUMBER"]
         ...
         #           awb_num = AirwaybillNumbers.objects.filter(airwaybill_number=airwaybill_num).update(status=1)
                    ...
                    pincode = record["REVPICKUP_PINCODE"]
                   ...
                   #            breadth=breadth, height=height, service_centre = servicecentre, original_dest = servicecentre)
                    ...
                    #return HttpResponse("%s, %s" % (rev_pickup,rev_pickup_flag))
                    if rev_pickup and rev_pickup_flag:
