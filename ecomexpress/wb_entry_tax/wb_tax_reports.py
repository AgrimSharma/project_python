...
def get_billing_date_str(bill_id, slash=False):
    bill = WBTaxBilling.objects.get(id=bill_id)
...
def get_bill_invoice_table(bill_id, total_amount, due_date):
    billing = CustomTaxBilling.objects.get(pk=bill_id)
...
def get_bill_soc_table_left(bill_id):
    billing =CustomTaxBilling.objects.get(pk=bill_id)
    soc_table_data = [
        ['WB Entrytax Charge', Decimal(billing.etax_charge).quantize(TW)],
        ['Entry tax Ecom Charge', Decimal(billing.etax_ecom_charge).quantize(TW)]]
...
def get_invoice_summary(bill_id, file_name, with_header=True):
    ...
    billing = CustomTaxBilling.objects.get(id=bill_id)
...
class GenerateOctroiPdf(GeneratePdf):
    ...
    def __init__(self, bill_id):
        ...
        self.billing = WBTaxBilling.objects.get(id=bill_id)
    ...
    def get_awb_total_table(self, ptype=None):
        billing = CustomTaxBilling.objects.get(pk=self.bill_id)

        awb_total_table = [
            ['Total Entry  Charge',	Decimal(billing.total_charge_pretax).quantize(TW)],
...
class GenerateOctroiExcel(GenerateReport):
    ...
    def __init__(self, bill_id):
        GenerateReport.__init__(self)
        self.bill_id = bill_id
        self.billing = CustomTaxBilling.objects.get(id=bill_id)
        self.row_num = 0
        self.col_heads = (
            'Sl No', #0
            'Cust Code',
