...
class Customer(models.Model):
    ...
    zone_label          = models.ForeignKey(ZoneLabel, blank=True, null=True, default=1)
...
class CashOnDelivery(models.Model):
    ...
    active      = models.BooleanField(blank=True, db_index=True, default = False)
    def __unicode__(self):
...
class ForwardFreightRate(models.Model):
    #customer    = models.ForeignKey(Customer)
    version    = models.ForeignKey(RateVersion)
    mode        = models.ForeignKey(Mode, default=1)
    slab        = models.IntegerField(max_length=5, default=500, help_text="Weight in gms")
    range_from  = models.IntegerField(max_length=6, default=0, help_text="Weight in gms", db_index=True)
    range_to    = models.IntegerField(max_length=6, default=999999, help_text="Weight in gms", db_index=True)
    org_zone    = models.ForeignKey(Zone, related_name = "ff_org_zone")
    dest_zone    = models.ForeignKey(Zone, related_name = "ff_dest_zone")
...
class CustomerAWBUsedLimit(models.Model):
    customer = models.ForeignKey(Customer)
    awb_limit = models.IntegerField(max_length=6, default=3000, help_text="Count of unused Awb Limit")
