...
def generate_invoice_pdf(bill_id, invoice_number='', with_header=False):
    print '5.1.1 start generating pdfs for bill ...',bill_id
    billing = Billing.objects.get(id=bill_id)
    bill_month = billing.billing_date.strftime('%B').upper()
    invoice_date = (billing.billing_date + datetime.timedelta(days=1)).strftime('%d-%b-%Y')
    man_power_date = billing.billing_date.strftime('01/%b/%Y')
    if with_header:
        full_file_name = PROJECT_ROOT + '/static/uploads/reports/dentsply_ebs_invoice_report_{0}.pdf'.format(bill_id)
    else:
        full_file_name = PROJECT_ROOT + '/static/uploads/reports/dentsply_ebs_invoice_report_without_head_{0}.pdf'.format(bill_id)
    ...
    table_data = [['Invoice number', invoice_number],
	          ['Date', invoice_date]]
    ...
    remark_para = '''
	    ...
	    services provided in the month of {0} 2014 at various cities as per list enclosed
	    </para>
	    '''.format(bill_month)
    ...
    manpower_data = [
        [Paragraph('''<para><b>Pick up-Origin</b></para>''', styles['LeftHeading']), 
         Paragraph('''<para><b>{0}</b></para>'''.format(man_power_date), styles['RightHeading']), 
         Paragraph('''<para><b>TOTAL</b></para>''', styles['RightHeading'])],
		     ['NOIDA', '1', '1'],
		     ['', '-', '-'],
		     ['', '-', '-'],
		     ['', '-', '-'],
