...
def get_pdsr_dispaly_data(date_str):
    ...
    day_data = Shipment.objects.filter(shipment_date=date_str).values("shipper__code").\
            annotate(
                ship_count=Count('id'),
    ...
    ddata = Shipment.objects.filter(shipment_date=date_str).values("shipper__code", "shipper__name", "product_type").\
            annotate(
                ship_count=Count('id'),
                total_cw=Sum('chargeable_weight'),
                freight_sum=Sum('order_price__freight_charge'),
    ...
    day_cod = Shipment.objects.filter(shipment_date=date_str).exclude(rts_status = 1).values("shipper__code").\
        annotate(cod_sum=Sum('codcharge__cod_charge'),)

    day_cod_negative = Shipment.objects.filter(shipment_date=date_str, rts_status=1).\
            values("shipper__code").\
            annotate(cod_sum=Sum('codcharge__cod_charge'),)

    # Monthly Report
    year, month, day = date_str.strip().split('-')
    month_data = Shipment.objects.filter(shipment_date__year=year, shipment_date__month=month, shipment_date__lte=date_str)\
    ...
    mdata = Shipment.objects.filter(shipment_date__year=year, shipment_date__month=month, shipment_date__lte=date_str)\
    ...
    month_cod_cod = Shipment.objects.filter(shipment_date__year=year, shipment_date__month=month, shipment_date__lte=date_str)\
            .exclude(rts_status = 1).values("shipper__code").\
        annotate(mcod_sum=Sum('codcharge__cod_charge'),)

    month_cod_cod_negative = Shipment.objects.filter(shipment_date__year=year,
    ...
    days = sum(1 for day in daygenerator if day.weekday() < 6)    #getting days excluding sundays
    projected_days_now = days  + 1

    print 'total projected days :', projected_days_now
    daygeneratormonth = (fromdate + datetime.timedelta(x + 1) for x in xrange((lastdate - fromdate).days))
    days = sum(1 for day in daygeneratormonth if day.weekday() < 6)    #getting days excluding sundays
    total_projected_days = days
    print 'month projected days :', total_projected_days
    ...
    def get_prodwise_row(day_data, month_data, day_cod_sum=0, month_cod_sum=0, ptype=None):
        ...
        account_in_charge = customer.activation_by.employeemaster.firstname + ' ' + \
                             customer.activation_by.employeemaster.lastname + '-' +\
                             str(customer.activation_by.employeemaster.employee_code)

        print "=============================" + account_in_charge
        pincode = customer.address.pincode
        try:
            zone = Pincode.objects.get(pincode=pincode).service_center.city.zone
        except Pincode.DoesNotExist:
            zone = ''
        cod_row = [code, account_in_charge, cname, website, zone]
        ...
        #print day_data.get('freight_sum') , day_data.get('fuel_sum') , day_cod_sum
        if day_data:
    ...
    def get_total_row(day_data, month_data, day_cod_sum=0, month_cod_sum=0):
        ...
        total_row = [code, account_in_charge, cname, website, zone]
        if day_data:
            #print 'daily total exists..'
            daily_total_cw = day_data.get('total_cw')
            others = day_data.get('vchc_sum') + day_data.get('to_pay_sum') + day_data.get('rto_sum') +\
                    day_data.get('sdl_sum') + day_data.get('reverse_sum') + day_data.get('sdd_sum')
            total = others + day_data.get('freight_sum') + day_data.get('fuel_sum') + day_cod_sum
            wt_ship = day_data.get('total_cw') / day_data.get('ship_count')
            yl_ship = total/day_data.get('ship_count')
    ...
    print 'got customer codes.... now entering loop'
    customers = list(Customer.objects.exclude(activation_status=False).values_list('id', 'code', 'name'))
...
def write_sales_comparison_to_excel(date_str):
    file_name = "/sales_comparison_report%s.xlsx"%(date_str)
    ...
    sheet.write(0, 3, "Sales Comparison Report ", header_format)
    sheet.set_column(2, 2, 25) # set column width

    sheet.write(1, 0, "Date ", header_format)
    sheet.write(1, 1, date_str)
    month = datetime.datetime.today().month
    prev_month = calendar.month_name[month - 1]
    sec_prev_month = calendar.month_name[month - 2]
    col_heads = ("Customer Code", "Account Incharge", "Customer Name", "Website", "Zone",
            sec_prev_month + " Rev", prev_month + " Rev", "Rev as of date", "Proj Rev",
            sec_prev_month + " pcs", prev_month + " pcs", "pcs as of date", "Proj pcs")
    ...
    prev_month = prev_month if prev_month else 12
    prev2_month = prev_month - 1
    prev2_month = prev2_month if prev2_month else 12

    prev_year = today.year - 1 if prev_month == 12 else today.year
    prev2_year = today.year - 1 if prev2_month == 12 else today.year
    ...
    #   totals_row[1] = map(sum, zip(totals_row[1], data[1][5:]))
        ...
        for ind, val in enumerate(data[2]):
            ...
            prev_billing =  Billing.objects.filter(customer__code=data[2][0],
                    billing_date__year=prev_year, billing_date__month=prev_month)\
                    .aggregate(cp=Sum('total_charge_pretax'), sc=Sum('shipment_count'))
            prev2_billing =  Billing.objects.filter(customer__code=data[2][0],
                    billing_date__year=prev2_year, billing_date__month=prev2_month)\
                    .aggregate(cp=Sum('total_charge_pretax'), sc=Sum('shipment_count'))
            prev_month_charge_sum = prev_billing.get('cp') if prev_billing.get('cp') else 0
            prev_month_ship_count = prev_billing.get('sc') if prev_billing.get('sc') else 0
            prev2_month_charge_sum = prev2_billing.get('cp') if prev2_billing.get('cp') else 0
            prev2_month_ship_count = prev2_billing.get('sc') if prev2_billing.get('sc') else 0

            sheet.write(row_num, 5, fix_decimal(prev2_month_charge_sum))
            sheet.write(row_num, 6, fix_decimal(prev_month_charge_sum))
            sheet.write(row_num, 9, fix_decimal(prev2_month_ship_count))
            sheet.write(row_num, 10, fix_decimal(prev_month_ship_count))
            sheet.write(row_num, 7, fix_decimal(data[2][21]))
            sheet.write(row_num, 11, fix_decimal(data[2][15]))
            sheet.write(row_num, 12, fix_decimal(data[2][25]))
            sheet.write(row_num, 8, fix_decimal(data[2][26]))
        totals_row[2] = map(sum, zip(totals_row[2], data[2][5:]))

        row_num += 1

    sheet.write(row_num, 2, 'GRAND TOTAL')
    prev_bill = Billing.objects.filter(billing_date__year=prev_year, billing_date__month=prev_month)\
            .aggregate(total_charge_pretax=Sum('total_charge_pretax'), shipment_count=Sum('shipment_count'))
    prev2_bill = Billing.objects.filter(billing_date__year=prev2_year, billing_date__month=prev2_month)\
            .aggregate(total_charge_pretax=Sum('total_charge_pretax'), shipment_count=Sum('shipment_count'))

    sheet.write(row_num, 5, round(prev2_bill["total_charge_pretax"], 0))
    sheet.write(row_num, 6, round(prev_bill["total_charge_pretax"], 0))
    sheet.write(row_num, 9, int(prev2_bill["shipment_count"]))
    sheet.write(row_num, 10, int(prev_bill["shipment_count"]))
    sheet.write(row_num, 7, fix_decimal(totals_row[2][21-5]))
    sheet.write(row_num, 11, fix_decimal(totals_row[2][15-5]))
    sheet.write(row_num, 12, fix_decimal(totals_row[2][25-5]))
    sheet.write(row_num, 8, fix_decimal(totals_row[2][26-5]))
