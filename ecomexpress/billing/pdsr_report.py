...
def get_pdsr_dispaly_data(date_str):
    ...
    day_data = Shipment.objects.using('local_ecomm').filter(
        shipment_date=date_str).exclude(shipper_id=12).values("shipper__code").\
            annotate(
                ship_count=Count('id'),
    ...
    ddata = Shipment.objects.using('local_ecomm').filter(
        shipment_date=date_str).exclude(shipper_id=12).values(
        "shipper__code", "shipper__name", "product_type").\
            annotate(
                ship_count=Count('id'),
                total_cw=Sum('chargeable_weight'),
                freight_sum=Sum('order_price__freight_charge'),
    ...
    day_cod = Shipment.objects.using('local_ecomm').filter(
        shipment_date=date_str).exclude(shipper_id=12).exclude(
        rts_status = 1).values("shipper__code").\
        annotate(cod_sum=Sum('codcharge__cod_charge'),)

    day_cod_negative = Shipment.objects.using('local_ecomm').filter(
        shipment_date=date_str, rts_status=1).exclude(shipper_id=12).\
            values("shipper__code").\
            annotate(cod_sum=Sum('codcharge__cod_charge'),)

    # Monthly Report
    year, month, day = date_str.strip().split('-')
    month_data = Shipment.objects.using('local_ecomm').filter(
        shipment_date__year=year, shipment_date__month=month, 
        shipment_date__lte=date_str).exclude(shipper_id=12)\
    ...
    mdata = Shipment.objects.using('local_ecomm').filter(
        shipment_date__year=year, shipment_date__month=month, 
        shipment_date__lte=date_str).exclude(shipper_id=12)\
    ...
    month_cod_cod = Shipment.objects.using('local_ecomm').filter(
        shipment_date__year=year, shipment_date__month=month, 
        shipment_date__lte=date_str).exclude(shipper_id=12)\
            .exclude(rts_status = 1).values("shipper__code").\
        annotate(mcod_sum=Sum('codcharge__cod_charge'),)

    month_cod_cod_negative = Shipment.objects.using('local_ecomm').filter(
        shipment_date__year=year, shipment_date__month=month,
        shipment_date__lte=date_str, rts_status=1).exclude(shipper_id=12).\
            values("shipper__code").\
            annotate(mcod_sum=Sum('codcharge__cod_charge'),)
