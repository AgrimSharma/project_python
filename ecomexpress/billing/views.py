        ...
        if request.POST.get("remittance_date") and request.POST.get("amount"):
            if Remittance.objects.filter(remitted_on = request.POST.get("remittance_date"), customer=customer):
                ...
                remittance = Remittance(
                    ...
                    remitted_on=request.POST.get("remittance_date"),
                    remitted_by=request.user)
                remittance.save()
                codcharges = CODCharge.objects.filter(
                    shipment__statusupdate__date__lte=request.POST.get("remittance_date"),
                    shipment__shipper=customer, shipment__statusupdate__status=2,
                    remittance_status=0)
...
def get_sum(numbers):
    return sum([x for x in numbers if x])

def daily_sales_report(request, report_date_str,status=0):
    ...
    sheet.write(3, 24,"Real Weight")
    sheet.write(3, 25,"Item description")
    sheet.write(3, 26,"Length")
    sheet.write(3, 27,"Breadth")
    sheet.write(3, 28,"Height")
    sheet.write(3, 29,"Pincode")
...
#    shipments = Shipment.objects.filter(inscan_date__year=report_date.year, inscan_date__month=report_date.month, inscan_date__day=report_date.day).order_by("shipper")
    ...
    for shipment in shipments:
	    ...
	    total_charge = get_sum([freight_charge, sdl_charge, sdd_charge, reverse_charge, fuel_surcharge, valuable_cargo_handling_charge, to_pay_charge, rto_charge, cod_charge])
	...
	if MinActualWeight.objects.filter(customer=shipment.shipper):
        ...
        total_valuable_cargo_handling_charge = get_sum([total_valuable_cargo_handling_charge, valuable_cargo_handling_charge])
        ...
        u = (shipment.airwaybill_number, shipment.order_number, shipment.product_type, shipment.inscan_date,  max_weight_dimension, shipment.collectable_value, shipment.declared_value, shipment.shipper.code, shipment.shipper.name, status,             freight_charge, sdl_charge, sdd_charge, reverse_charge, fuel_surcharge, valuable_cargo_handling_charge, to_pay_charge, rto_charge,cod_charge, total_charge, shipment.pickup.service_centre.center_shortcode, dest_code, original_dest, rts,shipment.shipext.original_act_weight,shipment.item_description,shipment.length,shipment.breadth,shipment.height,shipment.pincode)
        #report.append(u)
        row = row + 1
        for col, val in enumerate(u, start=0):
            style = datetime_style
...
def unremitted_report(request, cid):
    q = Q()
    if cid <> '0':
       customer = Customer.objects.using('local_ecomm').get(id=cid)
...
#  date_from = request.GET['from_date']
    ...
    shipment = Shipment.objects.using('local_ecomm').filter(q).exclude(rts_status__gte=1).filter(codcharge__remittance_status=0).distinct()
...
def provisional_report(request, queue_id):
    ...
    bill_ids = list(queue.bills.values_list('id', flat=True))
...
def add_to_report_queue(request):
    from billing.process_billing_reports import generate_async
    import traceback
    try:
        if request.method == 'POST':
            ...
            if form.is_valid():
                report_queue = form.save(bill_queue)
                generate_async(report_queue.id)
                message = 'success! report generation has started.'
...
def run_backdated(request):
     if request.POST:
         ...
         org_zone = Zone.objects.filter(id=zone_from)
         if org_zone :
            org_zone=Zone.objects.get(id=zone_from)
         else:
            org_zone = None
         dest_zone = Zone.objects.filter(id=zone_to)
         if dest_zone :
             dest_zone=Zone.objects.get(id=zone_to)
         else:
             dest_zone = None
         product= Product.objects.filter(product_name=prod_type)
         if product:
             product= Product.objects.get(product_name=prod_type)
         else:
             product = None
         check  = BackdatedBatch.objects.filter(product_type=product,date_from=from_date,date_to=date_to,customer=shipper,org_zone=org_zone,dest_zone=dest_zone,status=0)
         if not check:
             ...
             #return HttpResponse("%s %s" % (date_to,from_date))
         process_batch()
         ...
         batches=BackdatedBatch.objects.filter(added_on__gte=now).order_by('-added_on')
         pro_count = 0
         total_count = 0
         for b in batches:
             pro_count = pro_count + b.processed_count
             total_count = total_count + b.count
         pending = total_count - pro_count
         return render_to_response("billing/run_bacdated.html",
                               {'customer':customer,'pending':pending,'pro_count':pro_count,'total':total_count,'zone':zone,'batch':batches},
                               context_instance=RequestContext(request))
...
def process_batch():
         for batch in BackdatedBatch.objects.filter(status=0):
                 ...
                 count = 0
                 print ships.count(),batch[0].id
                 for s in ships:
                        #return HttpResponse("%s %s" % (s.airwaybill_number,batch[0].id))
                        add_awb(s.airwaybill_number,batch[0].id)
                        count += 1
                 batch.update(count=count,status=2)
...
def add_awb(awb,bid):
     ...
     if ship.rts_status == 1:
        ...
        rts=0
     check = BackdatedShipmentBillingQueue.objects.filter(airwaybill_number=awb)
     if not check:
          BackdatedShipmentBillingQueue.objects.create(backdated_bacth=batch,airwaybill_number=awb,shipment_date=ship.shipment_date,shipment_type=rts,product_type=ship.shipext.product)
     else:
          check.update(backdated_bacth=batch)



def process_queue():
       error_list=[]
       pending_batches = BackdatedBatch.objects.filter(status=2)
       #pending_ships = BackdatedShipmentBillingQueue.objects.filter(status = 2)
       for p in pending_batches:
         if p.count == p.processed_count :
             BackdatedBatch.objects.filter(id=p.id).update(status=1)
       pending_ships = BackdatedShipmentBillingQueue.objects.filter(backdated_bacth__status = 2,status=0)
       #print pending_ships.count()
       for sh in  pending_ships:
               ship = Shipment.objects.get(airwaybill_number=sh.airwaybill_number)
               if not ship.billing:
                 if not ship.shipext.product:
                     product = Product.objects.get(product_name = ship.product_type)
                     ShipmentExtension.objects.filter(shipment__airwaybill_number = ship.airwaybill_number).update(product=product)
                 try:
                     if ship.rts_status == 1:
                       rts_pricing(ship)
                     else:
                     ...
                     cnt=cnt+1
                     BackdatedShipmentBillingQueue.objects.filter(airwaybill_number=sh.airwaybill_number).update(status=1)
                     BackdatedBatch.objects.filter(id=btch.id).update(processed_count=cnt)
                 except:
                     print len(error_list)
                     error_list.append(ship.airwaybill_number)
...
@login_required
@json_view
def charge_verification(request):
    from billing.forms import ChargeVerificationForm
    data = []
    if request.GET:
        form = ChargeVerificationForm(request.GET)
        if form.is_valid():
            data = form.search()
    else:
        form = ChargeVerificationForm()
    html = render_to_string(
        'billing_admin/charge_verification.html',
        {'form': form, 'data': data},
        context_instance=RequestContext(request))
    return {'html': html}
...
def price_update(request):
    from service_centre.forms import PriceUpdateForm
    data = []
    if request.GET:
        form = PriceUpdateForm(request.GET)
        if form.is_valid():
            data = form.update()
    else:
        form = PriceUpdateForm()
    html = render_to_string(
        'billing_admin/price_update.html',
        {'form': form, 'data': data},
        context_instance=RequestContext(request))
    return {'html': html}
