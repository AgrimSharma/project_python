...
def get_bill_summary_filename(bill_id, name, ext, none_bill=False):
    if none_bill:
        bill_date_str = datetime.datetime.today().strftime('%Y_%m_%d')
    else:
...
def get_cust_code_summary_table(bill_id, preview=False):
    if preview:
        billing = BillingPreview.objects.get(pk=bill_id)
        billing_subcustomer = BillingSubCustomerPreview.objects.filter(
            billing=billing, billing_date=billing.billing_date).distinct()
    else:
        billing = Billing.objects.get(pk=bill_id)
        billing_subcustomer = BillingSubCustomer.objects.filter(
            billing=billing, billing_date=billing.billing_date).distinct()
...
def get_bill_summary_data(bill_id):
    ...
    if ppdbill.exists():
        ...
        ppdlist = [ 'PPD', ppdbill.billing.customer.code, ppdbill.billing.customer.name,
            ...
            ppdbill.sdd_charge, ppdbill.reverse_charge, '', ppdbill.valuable_cargo_handling_charge, ppdbill.cod_applied_charge,
            ppdbill.cod_subtract_charge, ppdbill.cod_applied_charge + ppdbill.cod_subtract_charge, '', ppdbill.total_charge_pretax, ppdbill.service_tax, ppdbill.education_secondary_tax, ppdbill.cess_higher_secondary_tax, ppdbill.total_payable_charge]
    else:
        ppdlist = [0]*22
        ppdlist[0] = 'PPD'
...
def generate_bill_summary_xls(bill_list):
    ...
    # add filename and set save file path
    if not bill_list:
        file_name = get_bill_summary_filename(None, 'bill_summary', 'xlsx', none_bill=True)
    else:
