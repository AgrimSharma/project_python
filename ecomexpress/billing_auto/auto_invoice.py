...
def manpower_reader(filename):
    workbook = xlrd.open_workbook(filename)
    worksheet = workbook.sheet_by_name("Manpower_Sep'14")
    num_rows = worksheet.nrows - 1
    num_cells = worksheet.ncols - 1
    curr_row = -1
    sdata = []
...
def generate_invoice_pdf(invoice_no, customer, filename, with_header):
    ...
    output_file_name = '/home/prtouch/workspace/auto/auto_invoice_%s.pdf' % tail_string
    img_location = '/home/prtouch/workspace/auto/Ecomlogo.png'
    ...
    def particular(amount, linehaul):
        manpower_para = '''
            <para>Manpower@Rs.25,000 (Location wise detail<br/>attached in 
            annexure)</para>'''
    ...
    # Gross total
    def gross_total(gross_total):
        gross_table = Table([[
            '',Paragraph('''<para><b>GROSS TOTAL</b></para>''',
            styles['bold_right']), 
            Paragraph('''<para><b>%s</b></para>''' % str(gross_total),
    ...
    #total
    def total(gross_total):
        servicetax, educess, hsecess = tax_calculation(gross_total)
        total = servicetax + educess + hsecess
    ...
    def inwords(gross_total):
        servicetax, educess, hsecess = tax_calculation(gross_total)
        total = servicetax + educess + hsecess
        words = mainfunction(int(total))
        inword_para = '''
            <para><b>In</b><br/><b>Words:</b></para>'''
        inword = Paragraph(inword_para, styles['Normal'])
        rsword_para = '''
            <para>Rs. %s<br/>only</para>''' % words
    ...
    # Account Head
    def account_head(filename):
        account_data = summary_reader(filename) 
    ...
    def annexure_of_manpower(filename):
        ...
        manpower_data = manpower_reader(filename) 
        manpower_table = Table(manpower_data, colWidths=(175, 75, 75, 75, 75, 100))
...
generate_invoice_pdf(1685, "tv18", '/home/prtouch/workspace/auto/sample.xlsx', True)
#generate_invoice_pdf(1685, "tv18", '/home/prtouch/workspace/auto/sample.xlsx', False)
